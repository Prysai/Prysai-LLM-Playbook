...
## 2026-08-10 follow-up pass

本轮重新加载当前工作树的页面并补做了首屏与路线交互检查：英文默认状态为 `lang=en`；中文切换后为 `lang=zh-CN` 且 URL 为 `?lang=zh`；路线 B 筛选只保留 B 组，`aria-pressed` 唯一为 `true`，实时提示同步为 7 章；390px 下菜单可打开、Escape 可关闭且焦点回到菜单按钮；320px 下无横向溢出；本轮控制台未观察到 error。截图抽查确认页面保持白底、黑字、单一红色信号、细线网格和左对齐层级的 Swiss 视觉方向。

本次 follow-up 仍不证明正文 Markdown 的目标部署渲染、屏幕阅读器、200% 缩放、色彩对比度、跨浏览器、真实移动设备或真实用户测试；页面状态保持 `candidate`。
