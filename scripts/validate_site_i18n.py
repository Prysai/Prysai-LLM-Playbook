...
# French is currently a candidate translation. These high-exposure labels are
# easy to regress when the generated dictionary is refreshed: literal English
# or false friends make the public shell harder to understand even when route
# coverage still passes. Keep the guard narrow and focused on user-visible copy.
FRENCH_COPY_GUARDS = {
    "luminaire": "fixture must be described as an offline support, not a light fixture",
    "chèque": "verification records must not use the banking term chèque",
    "Prise en charge des Skill": "supporting Skills label must be idiomatic French",
...
def translation_keys(app: str, language: str, html_keys: set[str]) -> set[str]:
    ...
    if language == "en":
        ...
        # The Traditional Chinese dictionary clones the effective Simplified
        # Chinese dictionary, then applies a final bracket-key override.
        # Reusing the effective zh key set keeps this validator aligned with
        # the runtime composition without duplicating a large object literal.
        base_keys = translation_keys(app, "zh", html_keys)
        blocks.extend(
            re.findall(
                r"Object\.assign\(copy\[['\"]zh-tw['\"]\], \{(?P<body>.*?)\}\);",
                app,
                re.DOTALL,
            )
        )
    ...
    blocks.extend(
        re.findall(
            (
                rf"Object\.assign\(copy\[['\"]{re.escape(language)}['\"]\], \{{(?P<body>.*?)\}}\);"
                if language == "zh-tw"
                else rf"Object\.assign\(copy\.{re.escape(language)}, \{{(?P<body>.*?)\}}\);"
            ),
            app,
            re.DOTALL,
        )
    ...
    if claim_audit_map:
        ...
        if claim_locale:
            blocks.append(claim_locale.group("body"))
    evidence_map = re.search(r"const\s+foundationEvidenceMapCopy\s*=\s*\{(?P<body>.*?)\n\};", app, re.DOTALL)
    if evidence_map:
        locale_key = rf"(?:['\"]{re.escape(language)}['\"]|{re.escape(language)})"
        evidence_locale = re.search(
            rf"\n\s{{2}}{locale_key}:\s*\{{(?P<body>.*?)\n\s{{2}}\}},",
            evidence_map.group("body"),
            re.DOTALL,
        )
        if not evidence_locale:
            evidence_locale = re.search(
                rf"(?:^|\n)\s{{2}}{locale_key}:\s*\{{(?P<body>[^{{}}]*)\}}",
                evidence_map.group("body"),
                re.DOTALL,
...
def validate_copy_initialization_order(app: str, errors: list[str]) -> None:
    ...
    for language in sorted(LOCALE_TOKENS - {"en", "zh"}):
        ...
        if not primary:
            ...
            continue
        early_override = re.search(
            (
                r"Object\.assign\(copy\[['\"]zh-tw['\"]\],"
                if language == "zh-tw"
                else rf"Object\.assign\(copy\.{language},"
            ),
            app[: primary.start()],
        )
        if early_override:
...
def validate_french_copy_guards(app: str, errors: list[str]) -> None:
    """Reject known high-exposure French false translations in the effective shell."""

    marker = "// Generated French interface dictionary"
    block = app.split(marker, 1)[1] if marker in app else app
...
def main() -> int:
    ...
    validate_french_copy_guards(app, errors)
    ...
    if not LOCALE_MANIFEST.is_file():
        ...
        manifest_tokens = set(re.findall(r'"([a-z]{2}(?:-[a-z]{2})?)": \{\n\s+"suffix": "(?:EN|ZH|ES|JA|KO|DE|ZHTW|FR)"', manifest_text))
        ...
        if manifest:
            if set(manifest.get("locales", {})) != LOCALE_TOKENS:
                errors.append("locale manifest locale records do not match the registered tokens")
    ...
    menu_tokens = set(re.findall(r'data-language-option="([a-z]{2}(?:-[a-z]{2})?)"', html))
