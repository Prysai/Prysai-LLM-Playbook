...
try {
  ...
  const localizedEvidenceStopTitles = {
    en: 'Stop',
    zh: '停止',
    es: 'Parar',
    ja: '停止',
  ...
  for (const locale of ['en', 'zh', 'es', 'ja', 'ko', 'de', 'zh-tw', 'fr']) {
    ...
    await evidenceMap.locator('[data-evidence-node="stop"]').click();
    assert.equal(await evidenceMap.locator('[data-evidence-node="stop"]').getAttribute('aria-pressed'), 'true', `${locale} evidence decision map Stop node is not selectable`);
    assert.equal(await evidenceMap.locator('[data-evidence-detail-title]').innerText(), localizedEvidenceStopTitles[locale], `${locale} evidence decision map Stop label is not localized`);
    assert.match(await evidenceMap.locator('[data-evidence-detail-link]').getAttribute('href'), new RegExp(`lang=${locale}$`), `${locale} evidence decision map route does not retain the selected language`);
    assert.deepEqual(
  ...
  const visualTriageLabels = {
    en: ['Text only', 'Tool or run claim', 'External side effect', 'Unknown or overconfident'],
    zh: ['只有文字', '工具或执行声称', '外部副作用', '未知或过度确定'],
    es: ['Solo texto', 'Afirmación de ejecución', 'Efecto externo', 'Desconocido o demasiado seguro'],
    ja: ['文章だけ', 'ツール／実行の主張', '外部への影響', '不明／過度に断定的'],
    ko: ['텍스트만 있음', '도구 또는 실행 주장', '외부 효과', '알 수 없음 또는 과도한 확신'],
    de: ['Nur Text', 'Tool- oder Ausführungsbehauptung', 'Außenwirkung', 'Unbekannt oder übermäßig sicher'],
    'zh-tw': ['只有文字', '工具或執行主張', '外部副作用', '未知或過度確定'],
    fr: ['Texte seulement', 'Action ou exécution annoncée', 'Effet externe', 'Inconnu ou trop affirmatif'],
  };
  ...
  const visualGuideGoalLabels = {
    ...
    es: ['Quiero empezar con una tarea segura', 'No sé qué demuestra el resultado', 'Necesito comprobar una afirmación', 'Quiero reutilizar el método'],
    ja: ['まず安全なタスクを試したい', 'この結果で何が言えるか分からない', '主張を確かめたい', '方法を別の課題でも使いたい'],
    ko: ['안전한 작업 하나부터 시작하고 싶다', '이 결과가 무엇을 증명하는지 모르겠다', '주장을 확인하고 싶다', '방법을 다른 작업에도 쓰고 싶다'],
    de: ['Mit einer sicheren Aufgabe beginnen', 'Ich weiß nicht, was dieses Ergebnis belegt', 'Eine Aussage überprüfen', 'Die Methode weiterverwenden'],
  ...
  const visualCapabilityLabels = {
    en: ['Observer', 'Safe user', 'Task designer', 'Workflow designer', 'Capability builder', 'Evidence reviewer', 'Team coach'],
    zh: ['观察者', '安全使用者', '任务设计者', '工作流设计者', '能力构建者', '证据审查者', '团队教练'],
    es: ['Observador', 'Usuario seguro', 'Diseñador de tareas', 'Diseñador de flujos de trabajo', 'Constructor de capacidades', 'Revisor de evidencia', 'Coach de equipo'],
    ja: ['観察者', '安全なユーザー', 'タスク設計者', 'ワークフロー設計者', '能力構築者', '証拠レビュアー', 'チームコーチ'],
  ...
  const visualReadingLabels = {
    ...
    zh: ['说清问题', '找到概念', '尝试一次有边界的行动', '检查记录', '说明边界', '在变化后的任务上尝试'],
    es: ['Nombrar el problema', 'Encontrar el concepto', 'Probar una acción acotada', 'Inspeccionar el registro', 'Declarar el límite', 'Probar una tarea distinta'],
    ja: ['問題を言葉にする', '概念を見つける', '範囲を区切って試す', '記録を確認する', '境界を示す', '条件を変えた課題で試す'],
    ko: ['문제 정하기', '개념 찾기', '범위가 정해진 행동 하나 시도하기', '기록 점검하기', '한계 밝히기', '조건을 바꾼 작업에서 시도하기'],
    de: ['Das Problem benennen', 'Den Begriff finden', 'Eine begrenzte Handlung testen', 'Den Beleg prüfen', 'Die Grenze nennen', 'Eine veränderte Aufgabe testen'],
    'zh-tw': ['說清楚問題', '找出概念', '嘗試一次有界線的行動', '檢查紀錄', '說明界線', '在變化後的任務上嘗試'],
    fr: ['Nommer le problème', 'Trouver le concept', 'Tenter une action délimitée', 'Inspecter le relevé', 'Énoncer la limite', 'Essayer sur une tâche différente'],
  };
  const visualBoundaryLabels = {
    en: ['Model proposes', 'Host decides authority', 'Tool executes', 'Person reads back', 'Evidence limits the claim'],
    zh: ['模型提议', '主机决定权限', '工具实际执行', '人读取结果', '证据限定结论'],
    es: ['El modelo propone', 'El entorno decide la autorización', 'La herramienta se ejecuta', 'Una persona comprueba el resultado', 'La evidencia limita la afirmación'],
    ja: ['モデルが提案する', 'ホストが権限を決める', 'ツールが実際に動く', '人が結果を読み返す', '証拠が主張の範囲を決める'],
    ko: ['모델이 제안하기', '호스트가 권한을 정하기', '도구가 실제로 실행되기', '사람이 결과를 다시 확인하기', '증거가 주장의 범위를 제한하기'],
    de: ['Das Modell schlägt vor', 'Der Host entscheidet über die Berechtigung', 'Das Tool wird ausgeführt', 'Ein Mensch liest das Ergebnis zurück', 'Belege begrenzen die Aussage'],
    'zh-tw': ['模型提議', '主機決定權限', '工具實際執行', '人重新核對結果', '證據限定主張'],
    fr: ['Le modèle propose', 'L’hôte décide de l’autorité', 'L’outil s’exécute', 'Une personne relit le résultat', 'Les preuves limitent l’affirmation'],
  };
  ...
  for (const locale of Object.keys(visualGuideLabels)) {
    ...
    assert.equal(await visualGuidePage.locator('.visual-card').count(), 17, `${locale} visual guide lost a teaching board`);
    ...
    assert.deepEqual(await visualGuidePage.locator('[data-visual-capability-nodes] button strong').allTextContents(), visualCapabilityLabels[locale], `${locale} visual guide capability levels are not localized`);
    assert.equal(await visualGuidePage.locator('[data-visual-capability-nodes] button').count(), 7, `${locale} visual guide capability ladder lost a level`);
    assert.equal(await visualGuidePage.locator('[data-visual-capability-fallback] li').count(), 7, `${locale} visual guide capability fallback lost a level`);
    ...
    assert.equal(await visualGuidePage.locator('[data-visual-reading-loop-title]').innerText(), visualReadingLabels[locale][0], `${locale} page practice selection is not localized`);
    assert.notEqual(await visualGuidePage.locator('[data-visual-reading-loop-image]').getAttribute('alt'), '', `${locale} page practice image has no alternative text`);
    assert.deepEqual(await visualGuidePage.locator('[data-visual-action-boundary-nodes] button strong').allTextContents(), visualBoundaryLabels[locale], `${locale} action boundary layers are not localized`);
    assert.equal(await visualGuidePage.locator('[data-visual-action-boundary-nodes] button').count(), 5, `${locale} action boundary map lost a layer`);
    assert.equal(await visualGuidePage.locator('[data-visual-action-boundary-fallback] li').count(), 5, `${locale} action boundary fallback lost a layer`);
    assert.notEqual(await visualGuidePage.locator('[data-visual-action-boundary-image]').getAttribute('alt'), '', `${locale} action boundary image has no alternative text`);
    assert.equal(await visualGuidePage.locator('[data-visual-action-boundary-title]').innerText(), visualBoundaryLabels[locale][0], `${locale} action boundary selection is not localized`);
    assert.deepEqual(await visualGuidePage.locator('[data-visual-triage-nodes] button strong').allTextContents(), visualTriageLabels[locale], `${locale} claim triage labels are not localized`);
    assert.equal(await visualGuidePage.locator('[data-visual-triage-nodes] button').count(), 4, `${locale} claim triage map lost a branch`);
    assert.equal(await visualGuidePage.locator('[data-visual-triage-fallback] li').count(), 4, `${locale} claim triage text fallback lost a branch`);
    assert.notEqual(await visualGuidePage.locator('[data-visual-triage-image]').getAttribute('alt'), '', `${locale} claim triage image has no alternative text`);
    assert.equal(await visualGuidePage.locator('[data-visual-triage-title]').innerText(), visualTriageLabels[locale][0], `${locale} claim triage selection is not localized`);
    ...
    await visualGuidePage.locator('[data-visual-capability-nodes] button').last().click();
    ...
    assert.match(await visualGuidePage.locator('[data-visual-reading-loop-link]').getAttribute('href'), new RegExp(`20-personal-codex-work-system-[A-Z]+\\.md&lang=${locale}$`), `${locale} page practice lesson loses its locale`);
    await visualGuidePage.locator('[data-visual-action-boundary-nodes] button').last().click();
    assert.equal(await visualGuidePage.locator('[data-visual-action-boundary-title]').innerText(), visualBoundaryLabels[locale][4], `${locale} action boundary selection does not update`);
    assert.equal(await visualGuidePage.locator('[data-visual-action-boundary-nodes] button').last().getAttribute('aria-pressed'), 'true', `${locale} action boundary selection is not exposed`);
    assert.match(await visualGuidePage.locator('[data-visual-action-boundary-link]').getAttribute('href'), new RegExp(`13-action-boundaries-[A-Z]+\\.md&lang=${locale}$`), `${locale} action boundary lesson loses its locale`);
    await visualGuidePage.locator('[data-visual-triage-nodes] button').last().click();
    assert.equal(await visualGuidePage.locator('[data-visual-triage-title]').innerText(), visualTriageLabels[locale][3], `${locale} claim triage selection does not update`);
    assert.equal(await visualGuidePage.locator('[data-visual-triage-nodes] button').last().getAttribute('aria-pressed'), 'true', `${locale} claim triage selection is not exposed`);
    assert.match(await visualGuidePage.locator('[data-visual-triage-link]').getAttribute('href'), new RegExp(`09-verification-and-recovery-[A-Z]+\\.md&lang=${locale}$`), `${locale} claim triage lesson loses its locale`);
  ...
  assert.equal(await visualGuidePage.locator('.visual-card').count(), 17, 'mobile visual guide loses teaching boards');
  ...
  await page.goto(`${origin}/site/?lang=fr`, { waitUntil: 'networkidle' });
  await noHorizontalOverflow(page, '360px French foundation teaching visuals');
  const narrowEvidenceMap = page.locator('#foundation-evidence-map');
  ...
  for (const [locale, suffix, expectedInspectLabel] of routeMapLocales) {
    ...
    // Chapter 15 and Lab 015 share the claim-to-evidence visual. Keep these
    // routes in the smoke test because a first-match visual map can silently
    // mask a more specific board with an older generic board.
    for (const [kind, pathStem] of [
      ['chapter 15', 'book%2Fchapters%2F15-research-track'],
      ['Lab 015', 'book%2Flabs%2Flab-015-evidence-delivery'],
    ]) {
      await page.goto(`${origin}/site/reader.html?path=${pathStem}-${suffix}.md&lang=${locale}`, { waitUntil: 'networkidle' });
      await page.locator('[data-reader-article][aria-busy="false"]').waitFor();
      assert.equal(await page.locator('[data-reader-visual-companion]').isVisible(), true, `${locale} ${kind} visual companion is missing`);
      assert.match(
        await page.locator('[data-reader-visual-companion] img').getAttribute('src'),
        /evidence-to-decision-stop-map-red-black\.svg$/,
        `${locale} ${kind} visual companion was masked by an older board`,
      );
