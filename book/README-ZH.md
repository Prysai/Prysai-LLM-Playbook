...
第 19–22 章已有草稿，当前状态统一为“草稿已写，待前测”。从[中文书籍目录](table-of-contents-ZH.md)进入每章的真实问题案例、评测夹具规范和研究档案；[`evals/task-set-v1.yaml`（机器可读文件，未做 `-ZH` 副本）](../evals/task-set-v1.yaml)已提交为 40 项固定夹具、覆盖 16 个轨道，但尚无模型运行日志，不能把它误读为已完成评测。
