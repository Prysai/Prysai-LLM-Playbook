...
这是第 2 章与实验 001 之间为新手准备的默认安全沙盒。它只提供一个故意不完整的 README、一次允许的本地改动和一个范围很小的检查器，完成后你再进入自己的项目。它不是第 23 章、新 Skill、Git 练习，也不是任何模型已经完成任务的证据。

如果你第一次打开项目文件夹或第一次运行检查器，这条路线就是为你准备的。你不需要安装新工具、注册账号，也不必拿真实项目冒险。目标会小到你能看见所有相关文件，并自己判断检查是否真的回答了问题。

## 问题

实验 001 要求你准备一个可丢弃的项目、找到真实的命令来源，并完成一次有边界的 README 改动。这些约束适合面向真实项目的练习，却可能让第一次实践陷入循环：新读者还没有安全项目，也不知道该相信哪个命令来源。

## 概念

固定练习材料把“练习方法”与“寻找合适项目”分开。它是虚构的、本地的、可丢弃的。唯一预期的改动是修正 README；检查器只读取这一个文件并给出一个简单结果。这样，你不用账号、网络、安装、Git、提交、推送、发布或个人数据，也能看见验收条件。

## 决定

如果你还没有可丢弃的本地项目，请使用项目自带的[第一次安全改动固定练习材料](../../examples/lab-001-v1/README-ZH.md)。把整个目录复制到 `.work/` 或其他临时目录；不要修改仓库里的原始材料，否则下一位读者看不到预先放入的错误。
...
第一次的结果应该是 `FIRST_SAFE_CHANGE_FAILED`。这是故意设计的起点，不是安装损坏。然后按照固定练习材料 README 中的任务卡，先检查 `seed/README.md`，提出最小改动方案；只有你自己认可方案后，才修改**这一份** README。再次使用同一个手动检查或可选命令。通过时的可选本地结果是 `FIRST_SAFE_CHANGE_OK`。

如果没有 Python，不要为了这条路线安装运行时或换用其他命令。使用不运行程序的检查，并记录 `check: manual required_readme_strings 3/3`。如果连可丢弃的本地副本也无法创建，请停止，改做纯文本的 First Win；不要把 GitHub 网页预览当成本地沙盒。
...
这个检查器最多只能说明：某个时刻，这份固定、虚构的 README 包含材料中要求的文字。检查通过并不等于 Git 操作成功、浏览器正常、账号权限正确、安全审查完成或已经学会。

## 失败与边界情况

不要为了得到通过而修改检查器、验收文件或其他路径。如果建议的修复需要安装、网络请求、密钥、账号、仓库操作或第二个文件，停止。那是新的决策，不属于这份固定练习材料。
...
下一项注册单元是[“实验 001：完成一次安全的 README 改动”](../labs/lab-001-first-safe-task-ZH.md)。它会在这份固定练习材料的基础上加入真实项目特有的命令来源、沙盒身份和恢复检查。该实验仍为 `draft / not_run`，中文译文尚未经过独立语言审校。
