...
如果你还没有可丢弃的本地项目，请使用项目自带的 [第一次安全改动夹具](../../examples/lab-001-v1/README.md)。把整个夹具复制到 `.work/` 或其他临时目录；不要修改仓库里的原始夹具，否则下一位读者看不到预先放入的错误。
...
下一项注册单元是“实验 001：完成一次安全的 README 改动”。它会在本夹具的基础上加入真实项目特有的命令来源、沙盒身份和恢复检查。该实验尚无简体中文译本，也仍是 `draft / not_run`；请从中文书稿目录中选择明确标为“英文源文件”的入口，不要把英文回退当成中文完成。
