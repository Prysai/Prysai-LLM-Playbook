...
2. <span id="unit-2-first-generation"></span>[Context, instruction, and a first generation](../chapters/02-first-safe-task-EN.md) — make the goal, material, constraints, and response shape explicit.
3. <span id="unit-3-visible-failures"></span>[Recognize visible failure modes](../communication-clinic-EN.md) — label omissions, unsupported additions, forced ambiguity, and overconfidence with source evidence.
