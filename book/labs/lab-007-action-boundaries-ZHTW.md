...
failure_variant: "把瀏覽器成功當成 token exchange，把一個已認證 host 當成目標 host，把一個組織的訪問當成另一個組織已安裝，或把驗證當成強制重灌許可權"
reflection: "哪種狀態最容易被‘已經登入’掩蓋？哪項檢查能增加證據而不擴大許可權？三個工作面如何改變回滾和審查？"
status: draft
last_verified: "未執行；真實三工作面實驗待執行；本檔案定義練習契約"
...
transfer_limitations: "本實驗不證明真實賬戶、Enterprise host、組織安裝、分支保護、connector、釋出平臺或遠端回滾可用"
---
...
根因。本實驗訓練的反應是定位階段、做最小檢查、在需要更大許可權時停止。
...
模型、工作面或許可權改變，必須在記錄中標記為 `not_comparable`，不能把差異
歸因給「行動邊界」本身。
...
回滾。預期只有本地檔案變化，賬戶、遠端和釋出狀態仍是 not_run。
...
| S-03 | Enterprise CLI 已認證但 PR 入口探測 github.com 並返回 401 | CLI host 和應用 host 可能不同 | GitHub 全部可用或 401 必然是倉庫許可權 | 只讀比較 host、remote 和入口 |
| S-04 | 能訪問一個組織但不能為第二個組織建立 installation | 身份、組織、installation、倉庫訪問是不同狀態 | 管理員許可權自動包含 installation | 記錄四個狀態，不申請安裝 |
...
每一步只改變一個條件。若同一檢查第二次失敗且沒有新的路徑、版本、許可權
...
檢查點。提交一張證據表，覆蓋任務邊界、工作面、基線、五個許可權欄位、症狀定位、
