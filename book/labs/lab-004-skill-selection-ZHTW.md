...
使用固定的候選版本。記錄來源、許可證、依賴、目標安裝範圍、權限、副作用、負責人、審查日期和回滾方式。除非後續任務明確授權，否則不要安裝或認證。
...
然後為一個簡單文字任務加入多個無關能力。只要某項能力增加的權限、依賴或驗證成本超過它帶來的具體價值，就拒絕它。
...
把這張比較表應用到一項研究或內容任務。哪項新依賴帶來了最高維護成本？刪除什麼仍不會降低最終證據的品質？
