...
不必从一个真正的网站或复杂代码库开始。新建一个临时文件 `status.md`，只放下面三行合成文字：
...
让 A 只收到“把这段内容整理成完成状态”。让 B 收到同一内容，再加上：**“只能修改 `status.md`；保留未知项；先列出计划；完成后检查是否仍有三行；不要把用户验收写成已完成。”** 两者都不联网、不提交、不改其他文件。

比较的不是谁写得漂亮，而是：有没有保留“尚未运行”、有没有说明实际修改、有没有给出可检查的结果。若 B 更清楚，也只能说明这一个协议在这一份合成文本上值得继续试；它不证明任何模型、团队或真实工程项目因此更高效。
