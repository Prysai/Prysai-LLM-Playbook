...
setup: "認証情報、ネットワーク、本番ファイル、不可逆なコマンドを使わない、一時的なローカルのテキスト課題"
...
新しい一時ディレクトリで行い、その中のローカル読み取りと可逆的書き込みだけを許可します。実際のリポジトリ、顧客資料、認証情報、ネットワーク、外部メッセージ、インストール、公開、push、破壊的削除、権限変更は使いません。

```text
read_root: 一時ディレクトリ
write_root: 一時ディレクトリと evidence/
...
一時ディレクトリに置いた文書フォルダのコピーで、`docs/guide/` の欠けたローカルリンクを探し `evidence/missing-links.md` に書く転移を行います。ソースは編集せず、ネットワークは使いません。
