...
## README 전에: 파일 없이 하는 프롬프트 비교

Git과 Codex, 터미널이 없어도 시작할 수 있습니다. 평소 쓰는 어떤 모델에서 아래의 가상
메모 또는 비밀이 없는 자신의 메모를 사용하세요.

```text
월요일: 초보자 안내의 오타 하나를 고쳤다.
화요일: 게시용 체크리스트를 작성했다. 검토자의 승인을 기다린다.
수요일: 올바른 명령어를 몰라 로컬 형식 검사를 실행하지 않았다.
다음: 게시 전에 필요한 검사를 Maya에게 확인한다.
```

같은 모델과 같은 메모로 새 대화 두 개를 만듭니다. 먼저 “이것을 좋은 프로젝트 업데이트로
...
네 가지 사실의 보존, 형식, unknown 명시, 쓸 수 있을 때까지의 수정 횟수를 비교합니다. 한 번의
응답으로 프롬프트나 모델이 “더 낫다”고 말하지 않습니다. 모델, 대화 기록, 입력, 기준을 바꾸면
`not_comparable`로 기록합니다. 이 입구는 파일이나 서비스 권한 없이 관찰 가능한 차이를 만듭니다.
