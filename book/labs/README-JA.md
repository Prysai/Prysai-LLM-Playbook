...
番号が小さいからという理由だけで Lab 001 から始める必要はありません。いまの自分に合う、
いちばん小さな観察可能な結果を選んでください。下のどの入口にも確認できる成果があり、
モデルが自信ありげに話すだけで信じる必要はありません。

| 今日やりたいこと | ここから始める | 止まる前に自分で確認できること |
| --- | --- | --- |
| コードやプロジェクトフォルダなしで、明確な依頼が役立つか確かめる | [Lab 001 の前半](lab-001-first-safe-task-JA.md#readme-の前file-を使わない-prompt-比較) | 同じ無害なメモへの二つの回答と、短い比較記録 |
| GPT、作業面、ツール、Agent がそれぞれ何をするか理解する | [Lab 011](lab-011-gpt-codex-boundaries-JA.md) | 提案された行動と、実行・確認済みの行動を分ける境界図 |
| 「これを手伝って」を、他の人も確認できる依頼に変える | [Lab 002](lab-002-task-protocol-JA.md) | 目的、情報源の境界、許可された行動、受け入れ、停止条件があるタスクカード |
| 固定した情報源の調査回答を、完全だと装わずに確認する | [Lab 008](lab-008-research-question-JA.md) | 情報源一覧、範囲を限定した結論、明示した未確認事項 |
| 小さなローカルファイル変更を行う | [最初の安全な変更](../routes/first-safe-change-JA.md) の後に [Lab 001 の作業スペース部分](lab-001-first-safe-task-JA.md#この-lab-の目的) | 破棄可能なコピー内の、確認済み README diff と限定したローカルチェック |

今日使えるのがチャット画面だけなら、最初の行だけで十分です。カタログに追いつくためだけに
ツールを入れたり、アカウントを作ったり、本物のプロジェクトを使ったりしないでください。
破棄できるフォルダ、許可された一つの対象、残す証拠を言えるようになってから、作業スペースの
練習へ進みます。
