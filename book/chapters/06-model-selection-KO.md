...
**상태:** `candidate`. 아래의 비교 프로토콜은 작성되어 있고 출처 범위도 정해져 있지만,
이 리포지토리는 고정 작업 세트를 아직 실행하지 않았습니다. 모델 성능, 비용, 지연 시간,
용량, 안정성, 전체 순위는 모두 여전히 `not_run` 상태입니다.

## 이 장이 해결하는 문제

모델 선택은 흔히 “최고의 모델을 쓰세요”라는 슬로건으로 끝납니다. 실제 업무에는 더 구체적인
질문이 필요합니다.

> 이 작업, 이 작업면, 이 provider, 이 컨텍스트와 도구 세트, 이 권한 경계, 이 시간
> 예산과 수용 기준(rubric)에서 최소 요구 조건을 충족하는 후보는 무엇인가?
> 그리고 시험을 넓혀도 될 만큼 근거가 충분한가?

후보가 선택한 작업면에서 사용할 수 없거나 두 실행의 입력, 도구, 권한, reasoning 설정이
다르면 공정한 모델 비교가 성립하지 않습니다. 보기 좋은 demo가 보여 주는 것은 한 구성이
한 번 결과를 냈다는 사실뿐입니다. 그것만으로 보편적인 순위나 전체적인 가치를 정할 수는
없습니다.
...
- 모델 ID, provider, reasoning effort, 컨텍스트, 도구, 권한, 수용 기준을 서로 다른
  비교 변수로 분리한다.
- 특정 후보에 유리하도록 조건을 바꾸지 않고 저위험 세 작업 스모크 비교(smoke comparison)를
  실행한다.
- 용량 제한, provider 불일치, 장시간 대기 실패를 증거로 보존한다.
...
GitHub Issue와 기타 공개 토론을 수집합니다. 이 보고들은 증상에 대한 기록이지, 공식 진단이나
로컬 재현이 아닙니다. 모델 선택이 잘못될 때 사람들이 어떤 가정을 세우는지 보여 주기 때문에
가치가 있습니다.

| 공개 증상 | 신고자가 관찰한 것 | 증명하지 **않는** 것 | 가장 먼저 취할 안전한 대응 |
|---|---|---|---|
| 모델 선택기가 `model`을 바꾸지만 사용자 지정 `model_provider`는 남겨 둠 | 화면에 보이는 모델과 실제 적용된 provider가 잘못된 조합이 될 수 있음 | 선택기, provider, 모델이 전반적으로 고장 났다는 것 | 실제 적용된 `model`과 `model_provider`를 함께 읽고, 수정하기 전에 민감한 값을 가린 설정 diff를 보존 |
| 선택한 모델이 용량 한도에 도달함 | 작업이 완전한 결과를 내기 전에 멈추고, 이후 prompt가 부분 상태를 만날 수 있음 | 모델 품질이 낮다는 것, 또는 재시도하면 첫 시도가 끝났다는 뜻이라는 것 | checkpoint, diff, 로그, 테스트를 저장하고 계속하기 전에 상태를 분류 |
...
원본 링크, 날짜, 버전, 증거 수준, 불확실성 메모는
[모델 선택 연구 기록](../evidence-library-KO.md#source-notes)에
있습니다. 이 프로젝트는 그 보고들의 명령이나 해결 방법을 실행하지 않았습니다.

### 실제 보고서를 전설로 만들지 않고 활용하는 법

각 증상에 대해 다음 네 가지 라벨을 분리해 기록하세요.

1. **사용자 보고(User report):** 어떤 사람이 이름이 알려진 환경에서 이런 일이 일어났다고
   말한 내용.
2. **독립 보고(Independent report):** 다른 사용자도 비슷한 증상을 설명하는지 여부.
3. **공식 확인(Official confirmation):** 메인테이너(maintainer)의 답변, 공식 문서, 릴리스
   노트 등 일차 출처의 증거.
4. **Playbook 증거:** 이 프로젝트가 실제로 재현한 내용.

위 세 예시에는 처음 두 라벨이 붙을 수 있지만, 이 프로젝트에는 이를 보장된 해결책으로
격상할 로컬 재현도 공식적인 근본 원인 확인도 없습니다. 따라서 행동도 달라져야 합니다.
마법의 설정을 약속하는 대신 증거를 보존하고 다음 검사를 좁혀야 합니다.

## 1. 모델 선택은 구성 결정이다

### 가용성이 품질보다 먼저다

두 개의 별도 확인 단계(gate)를 사용하세요.
...
각 화살표는 서로 다른 주장을 뜻합니다. 모델이 공식 페이지에 설명되어 있어도 어떤
account에서는 사용할 수 없을 수 있습니다. 선택기에 나타나도 provider가 요청을 받는 순간
실패할 수 있습니다. 텍스트 응답이 성공해도 작업에 필요한 파일, 터미널, 브라우저,
connector를 사용할 수 있다는 뜻은 아닙니다.
...
`not_observed`는 유효한 결과입니다. 검사를 수행하지 않았거나 사용할 수 있는 증거를 남기지
...
이것들은 제품 설명이지 Playbook 벤치마크 결과가 아닙니다. 공식 페이지는 reasoning
effort를 높이면 복잡한 작업이 나아질 수 있지만 시간이 더 걸리고 token을 더 많이 쓴다고
경고합니다. 수용 기준을 만족하는 가장 낮은 effort에서 시작하고, 작업에 더 많은 계획,
분석, 확인이 필요할 때만 높이세요. 그 설정을 실행 기록에 남기세요.
