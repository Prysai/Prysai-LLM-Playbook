...
**狀態：** `candidate`。下文的比較協議已經寫明，也標出了來源邊界，但本儲存庫尚未執行固定任務集。模型效能、成本、延遲、容量、穩定性和任何總體排名仍為 `not_run`。

## 本章要解決的問題

談模型選擇時，人們常用一句口號替代判斷：「用最好的模型。」真實工作需要問得更具體：

> 針對這項任務，在這個工作面、這個提供商、這組上下文與工具、這條權限邊界、這個時間預算和驗收標準下，哪個候選項能達到最低要求？我們是否已有足夠證據擴大試驗？

若候選模型在所選工作面不可用，或兩次執行使用了不同輸入、工具、權限或推理設定，就無法進行公平的模型比較。漂亮演示至多說明某個設定曾產生一個結果，不能據此建立通用排名或總體性價比結論。
...
- 先選擇任務和工作面，再選擇模型；
- 在實際帳戶、工作區、提供商和工作階段中核驗模型可用性，而不是根據目錄或選擇器推斷；
- 將模型 ID、提供商、推理強度、上下文、工具、權限和驗收標準視為不同的比較變數；
- 執行低風險的三任務快速比較，而不為「救回」某一個候選項而改變條件；
- 將容量不足、提供商不匹配和長時間等待失敗儲存為證據；以及
- 說明實驗能證明什麼、不能證明什麼，以及何時該停止。

## 現實問題入口：模型選擇會在日常情境中失敗

專案的 [Codex 現場研究](../evidence-library-ZHTW.md#source-notes) 彙集公開 GitHub Issue 和其他公開討論。這些記錄描述的是症狀，不是官方診斷或本機復現；它們的價值在於暴露模型選擇出錯時人們常作出的假設。

| 公開症狀 | 報告者觀察到什麼 | 它**不能**證明什麼 | 第一個安全響應 |
|---|---|---|---|
| 模型選擇器更改了 `model`，卻保留自訂 `model_provider` | 可見模型和實際提供商可能構成無效配對 | 選擇器、提供商或模型在所有環境中都已損壞 | 同時讀取實際生效的 `model` 與 `model_provider`；更正前保留去識別化的設定 diff |
| 所選模型達到容量上限 | 任務在得到完整結果前停止，後續提示可能面對部分狀態 | 模型品質低，或重試就代表第一次已經完成 | 儲存檢查點、diff、日誌和測試；繼續前先分類該狀態 |
| Windows 命令持續顯示 `Working` | 介面顯示仍在活動，卻沒有可驗證輸出 | 格式化器、Agent 或模型仍在進行有用工作 | 採用超時/停止規則，安全中斷，檢查 worktree，只重跑有界檢查 |

原始連結、日期、版本、證據等級和不確定性說明見[模型選擇研究記錄](../evidence-library-ZHTW.md#source-notes)。本專案沒有執行這些報告中出現的命令或替代方案。
...
3. **官方確認：** 維護者回覆、官方文件、發行說明或其他來自第一方的證據。
4. **Playbook 證據：** 本專案實際復現了什麼。

上面三個例子可能已有前兩類標籤，但專案沒有本機復現，也沒有官方根因確認，不能把它們升級為保證有效的修復方案。因此正確做法是儲存證據、縮小下一步檢查，而不是承諾一個「神奇設定」。
...
每一個箭頭對應不同宣告。官方頁面可以描述某模型，但它仍可能不對某個帳戶開放；它也可能出現在選擇器中，卻在請求到達提供商時失敗。一次成功的文字回覆同樣不能證明任務所需的檔案、終端機、瀏覽器或連接器都可用。
...
`not_observed` 是有效結果：它表示檢查沒有執行，或沒有留下可用證據。這比用猜測填滿表格安全。

### 產品定位只是起始假設

在 2026-08-11 的來源檢查中，官方 Codex 模型頁大致這樣描述推薦的 GPT-5.6 選項：

| 官方定位 | 合理的起始假設 | 仍需測試什麼 |
|---|---|---|
| Sol：複雜、開放式工作，提供更多分析和打磨 | 當模糊性、判斷或高價值審查佔主導時嘗試它 | 在你的任務集上的首輪透過率、時長、成本、穩定性和工具行為 |
| Terra：務實的日常主力 | 對需要強推理和工具使用的一般工作嘗試它 | 在實際約束下是否達到你的驗收閾值 |
| Luna：清晰、可重複、高吞吐工作 | 對提取、分類、轉換和結構化摘要嘗試它 | 加上上下文、提供商、推理強度與審查成本後，結果是否仍可接受 |

這些是產品描述，不是 Playbook 的基準測試結果。官方頁面同樣提醒：更高推理強度可能改善複雜工作，但會花更久、使用更多 token。先用滿足驗收標準的最低強度；只有任務確實需要更多規劃、分析或檢查時才提高，並將該設定記入執行記錄。

`Max` 與 `Ultra` 不是免費的品質標籤。官方頁面將 Max 描述為給單一任務更多推理時間，將 Ultra 描述為對可拆分複雜工作使用子代理。它們改變工作流程與資源邊界，因此 Ultra 執行不能與單一 Agent 執行當作純模型比較。
...
官方文件描述本機桌面端、CLI 和 IDE 預設值共用 `config.toml` 路徑，而 Cloud 對話有不同的預設模型邊界。設定檔只構成設定證據：在認定元組已生效前，重新查看實際提供商和模型，再做一次無害請求。
...
一個候選項通過提取任務，仍可能不適合多檔案修復或高風險證據審查。驗收標準必須匹配任務類別。

### 鎖定工作面與風險邊界

選擇能提供所需證據的最小環境。任務不需要遠端執行時，將合成或去識別化輸入保留在本機；目前未提交的工作必須隔離時，使用可丟棄的 Worktree；僅在儲存庫、環境、網路、秘密資訊和審查路徑都已核准且可觀察時使用 Cloud。

模型選擇無法補償缺少的檔案、不可用的連接器、錯誤的 checkout 目錄或未授權寫入。環境錯誤時，應停在工作面決策，而不是在不平等條件下「測試」模型。
...
不要只為一個候選項改變提示詞、增加上下文、授予工具、提高強度或放寬權限。若任務契約變化，遞增版本，並在新版本下重跑兩個候選項。
...
凍結 `task_set_version: three-task-smoke-v1`、兩張候選卡、一份驗收標準、原始輸出位置、日誌位置，以及針對不可用、容量中斷、權限不一致、輸入漂移或工具版本漂移的停止條件。
...
3. 按相同任務順序、相同輸入和相同驗收標準執行候選項 A 與 B。
...
| 失敗變體 | 為什麼結果不可比較 | 安全處理 |
|---|---|---|
| 候選項在所選工作面不可見或不可呼叫 | 不存在同一工作面的對照執行 | 記錄 `surface_available: no` 或 `not_observed`；停止該候選項，不將不可用性計作模型品質 |
| 模型選擇器與提供商不一致 | 請求可能沒有使用預期模型 | 儲存去識別化後的實際設定 diff；修正元組，或把比較改為提供商／工作流程測試 |
...
面對容量或長時間等待失敗，正確做法不是「一直點選到成功為止」。應該儲存最後已知狀態，先判斷任務是完成、部分完成，還是狀態未知，再選擇有界的復原方式。新對話可以作為復原工作面，卻不會繼承舊對話的完成證明。
...
- 哪項任務改變了擴大/停止決定？
- 哪個差異可能來自模型，哪個可能來自工作面、提供商、上下文、工具、權限、容量或審查者？
- 在什麼地方，更快或更便宜的輸出仍會不滿足驗收標準？
...
凍結新的任務集版本和領域驗收標準。不要把模型選擇或三任務結果直接複製到新領域；說明哪些結論仍侷限於該任務，哪些主張必須放棄。

## 本章證據

預期交付物是兩張候選卡、凍結的任務集與驗收標準、初始原始執行及任何受控返工、一張比較表、型別化錯誤記錄，以及擴大/停止決定。在這些記錄存在前，本章實驗必須保持 `not_run`；官方定位和一次演示都不能替代評測證據。

## 來源與維護邊界

| 事實或方法邊界 | 來源 | 查閱日期 | 適用範圍 | 負責人 / 下次複核 |
|---|---|---:|---|---|
| 官方模型定位、推理指導、本地預設值、Cloud 模型邊界和棄用說明 | [Codex models](https://learn.chatgpt.com/docs/models.md) | 2026-08-11 | 查閱當日的官方文件；不是帳戶層級的可用性證明或基準測試 | `facts-maintainer` / 2026-09-11 |
| CLI 工作面與本地倉庫工作流程 | [Codex CLI](https://learn.chatgpt.com/docs/cli.md) | 2026-08-11 | 官方 CLI 文件；不是本會話的實際設定 | `facts-maintainer` / 2026-09-11 |
...
- [ ] 我能在命名模型前定義任務、風險、工作面、提供商和驗收標準。
- [ ] 我能記錄實際可用性證據，而不是從模型目錄、設定值或選擇器標籤推斷存取權限。
- [ ] 我能為兩個候選項填寫模型、提供商、推理強度、上下文、工具、權限、成本基準和任務集版本。
