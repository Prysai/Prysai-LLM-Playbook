...
**狀態：** `candidate`。下文的比較協議已經寫明並有來源邊界，但本儲存庫尚未執行固定任務集。模型效能、成本、延遲、容量、穩定性和總體排名仍為 `not_run`。

## 本章要解決的問題

談模型選擇時，人們常用一句口號替代判斷：「用最好的模型。」真實工作需要問得更具體：

> 針對這項任務，在這個工作面、這個提供商、這組上下文與工具、這條權限邊界、這個時間預算和驗收量表下，哪個候選項能達到最低要求？我們是否已有足夠證據擴大試驗？

若候選模型在所選工作面不可用，或兩次執行使用了不同輸入、工具、權限或推理設定，就不存在乾淨的模型比較。漂亮演示至多說明某個設定曾產生一個結果，不能建立通用排名或總體性價比結論。
...
專案的 [Codex 現場研究](../evidence-library-ZHTW.md#source-notes) 彙集公開 GitHub Issue 和其他公開討論。這些記錄是症狀，不是官方診斷或本機復現；它們的價值在於暴露模型選擇出錯時人們常作出的假設。
...
| 所選模型處於容量限制 | 任務在得到完整結果前停止，後續提示可能面對部分狀態 | 模型品質低，或重試就代表第一次已經完成 | 儲存檢查點、diff、日誌和測試；繼續前先分類該狀態 |
...
上面三個例子可能已有前兩類標籤，但專案沒有本機復現，也沒有官方根因確認，不能把它們升級為保證有效的修復方案。因此行動應是儲存證據、縮小下一步檢查，而不是承諾一個「神奇設定」。
...
每一個箭頭對應不同宣告。官方頁面可以描述某模型，它仍可能不對某個帳戶開放；它也可能出現在選擇器中，卻在請求到達提供商時失敗。一次成功的文字回覆同樣不能證明任務需要的檔案、終端機、瀏覽器或連接器可用。
...
這些是產品描述，不是 Playbook 的基準測試結果。官方頁面同樣提醒：更高推理強度可能改善複雜工作，但會花更久、使用更多 token。先用滿足驗收量表的最低強度；只有任務確實需要更多規劃、分析或檢查時才提高，並將該設定記入執行記錄。
...
凍結 `task_set_version: three-task-smoke-v1`、兩張候選卡、一份驗收量表、原始輸出位置、日誌位置，以及針對不可用、容量中斷、權限不一致、輸入漂移或工具版本漂移的停止條件。
...
面對容量或長時間等待失敗，實務上的應對不是「一直點選到成功為止」。應該儲存最後已知狀態，先判斷任務是完成、部分完成，還是狀態未知，再選擇有界的復原方式。新對話可以作為復原工作面，卻不會繼承舊對話的完成證明。
