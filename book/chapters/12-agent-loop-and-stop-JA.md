...
使い捨てディレクトリに、順番を入れ替えた三行を持つ `input.txt` を作ります。空でない行を並べ替えて `output.txt` に書くタスクです。読み取りと書き込みはこのディレクトリだけに限定し、ネットワーク接続とインストールは禁止します。
...
納品メモでは、`Completed`（完了したこと）、`Observed actions`（観測した操作）、`Evidence`（証拠）、`Acceptance coverage`（受け入れ条件の充足範囲）、`Not proven`（証明していないこと）、
`Unresolved`（未解決）、`Retry budget`（再試行予算）、`Stop or next decision`（停止または次の判断）を分けます。英語のラベルを残す場合も、最初に日本語で意味を示します。「すべてのテストに合格した」とだけ書くのは、
