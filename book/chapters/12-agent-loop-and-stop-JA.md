...
納品メモでは、`Completed`（完了）、`Observed actions`（観測した操作）、`Evidence`（証拠）、`Acceptance coverage`（受け入れ条件の充足範囲）、`Not proven`（証明していないこと）、
`Unresolved`（未解決）、`Retry budget`（再試行予算）、`Stop or next decision`（停止または次の判断）を分けます。「すべてのテストに合格した」とだけ書くのは、
