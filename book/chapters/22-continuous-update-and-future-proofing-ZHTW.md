...
> 練習只在可丟棄副本或隔離分支中執行。它不使用生產訪問、真實憑據、推送、釋出或外部批次替換。

## 本章要解決的問題

Codex 入口、模型、推理設定、許可權、Skill 分發和外部服務都可能變化。如果一套工作流沒有來源、範圍、複核日期、遷移計劃或回滾路徑，今天還能正常工作的它，幾個月後就可能誤導人。持續維護不是競相採納每一個新功能的競賽，而是一種有紀律的方式，用來判斷什麼穩定、什麼易變、什麼必須重新核查，以及舊版本應在何時保留、阻塞、遷移或下線。

## 真實問題入口

FP-01 涉及身份驗證流程的迴歸，FP-06 涉及 Skill 發現的邊界，FP-10 涉及一條可能看似停滯的驗證命令，它們都是公開的使用者報告。它們不能取代當前的第一方文件或本地復現，而是練習影響分析、版本決策、停止與回滾的有用素材。
...
- 用 `claim`、來源、訪問日期、範圍、負責人、複核日期和 `claim_status` 記錄每項易變主張；
- 當模型、工具或 Skill 變化時，建立影響矩陣以及最小化的遷移與回滾計劃；
...
| 層級 | 示例 | 維護方式 |
|---|---|---|
| 穩定原則 | 上下文影響理解；工具改變行動空間；證據支撐完成宣告 | 教學、實驗與邊界複核 |
| 產品用法 | Codex 入口、Skill 呼叫、許可權模式、配置 | 對照具體的第一方頁面重新核查 |
| 領域方法 | 工程、研究、營銷、文件與資料工作流 | 練習任務與人工複核 |
| 例項事實 | 模型 ID、價格、額度、引數和第三方 API 行為 | 繫結帶日期的來源；必要時遷移或移除 |
...
| 來源相互衝突、賬戶範圍不清，或觀察到的行為與來源衝突 | `disputed`；暫停確定性的措辭 | 標記未知項並指派複核負責人；不釋出確定的結論 |
| 來源不可用，且沒有替代證據 | `stale`；警告或臨時阻塞 | 不要再把舊主張當作當前事實呈現 |
| 許可證或安全條件不再允許該能力，且沒有安全的替代方案 | `removed`；下線該能力 | 保留遷移說明與恢復資訊 |
| 存在相容的替代方案，且遷移與評測透過 | `current`；釋出遷移說明 | 寫明舊範圍、替代路徑、證據與下次複核 |
...
發現變化
    → 判斷影響與風險
    → 定位受影響的章節、Skills、Labs、任務集、prompts 和許可權
    → 閱讀來源或收集有邊界的執行時證據
    → 做出最小化的安全改動
    → 重跑相關檢查與評測
    → 獲得全新上下文的複核
...
對於模型或 Skill 遷移，要重新核查任務集的首次透過行為、錯誤型別、上下文、工具、許可權、觸發器、輸出格式、許可證、維護者和故障恢復。來源重新整理只會在其宣告範圍內更新一項主張；它不能證明賬戶級訪問、執行時行為、部署或團隊成效。
...
`example.invalid` 是刻意不可用的，因此初始主張必須保持 `disputed`：不要訪問它、執行它的指令，或把它當作真實的產品證據。儲存目標檔案的 SHA-256、基線目錄清單、變更前的 diff 和執行 ID。不要訪問生產環境、使用真實憑據、推送、釋出、執行批次替換，或連線外部服務。
...
2. 建立一張至少包含以下各行的影響矩陣：
   ...
   | 許可權說明 | 範圍或審批 | 越權 | 靜態複核 | 許可權矩陣 | 安全負責人 | pending |
   | 任務集 | 任務或禁止動作 | 迴歸缺口 | 建立新版本 | 任務結果 | 評測負責人 | pending |

3. 只修改紙面 fixture 中必要的主張狀態和說明性註釋。不要把未經證實的替代行為寫成事實。
...
證據包必須包含主張 YAML、來源快照或來源不可用記錄、訪問日期與範圍、影響矩陣、變更前後的雜湊、diff、檢查輸出、狀態轉換的原因、未證實項列表、負責人、下次複核日期和回滾說明。必需項一共十件：claim、source、scope、owner、`next_review`、基線雜湊、變更後雜湊/diff、影響矩陣、驗證日誌和未證實項列表。缺少任何一件，都意味著更新閉環尚未完成。

回滾必須能在不訪問生產環境的情況下執行：從變更前的雜湊恢復臨時副本，或者丟棄臨時副本或分支。保留變更前後的 diff 和結果。“檔案看起來已恢復”不是回滾證據。

### 失敗案例與邊界

刻意製造一次失敗：把一個新的模型名或動作名替換進所有文件，卻不更新任務集、範圍、來源、許可權或遷移說明。停止這種做法，在臨時副本中保留失敗的 diff，恢復基線雜湊，並把被遺漏的下游使用方補進影響矩陣。如果來源相互衝突、許可不明確、負責人缺失或評測未執行，就讓主張保持 `disputed` 或 `stale`、工作保持 `blocked`；不要釋出它。
...
- 更大的目錄並不能證明能力系統得到了改進；一項新能力必須增加證據、價值或覆蓋範圍。
- 可訪問的官方來源並不能證明某個本地入口、賬戶或組織已啟用該功能。
- 使用者報告是一條研究條目，而不是自動成立的官方根因。
- 批次名稱替換不能替代影響分析、評測、許可權複核和許可證複核。
- `claim_status: current` 只表示該項主張在其宣告範圍內有當前來源，並不表示章節、Skill、實驗、部署或執行時是 `verified`。
- 成功的構建、準備好的包或成文的遷移都不是生產行為或團隊成效的證據，除非該證據確實存在。

## 遷移任務

選擇一個真實但脫敏的外部 Skill 候選。用主張記錄和影響矩陣，把它從“未複核”推進到 `blocked` 或“適配候選”。說明它在許可證、依賴、觸發器、許可權、風險、負責人和評測證據方面還缺少什麼。不要僅僅因為名字看起來合適就批准它。

## 驗收清單

- [ ] 我能區分穩定原則、產品用法、領域方法和例項事實。
- [ ] 每項易變主張都有 `claim`、`source`、`checked_at`、`applies_to`、`owner`、`next_review` 和 `claim_status`。
- [ ] 我能用影響矩陣在章節、Skills、Labs、任務集和許可權說明中定位下游影響。
- [ ] 我能解釋主張狀態與內容成熟度之間的區別。
...
生命週期、影響矩陣、回滾和證據門屬於本專案的方法論。模型名稱、ID、入口、推理設定、Skill 行為和許可權邊界是易變的產品事實，必須對照當前的第一方來源重新核查。

```yaml
- claim: "模型名稱、ID、入口、推理設定和可用性以當前官方 Models 文件為準"
  source: "https://learn.chatgpt.com/docs/models.md"
  checked_at: "2026-08-09"
  applies_to: "官方文件所述 Codex 與 ChatGPT 入口、賬戶範圍和版本範圍"
  owner: "內容與模型評測維護者"
...
- claim: "Skill 的發現、呼叫、分發和 Plugin 組合是易變的產品事實"
  ...
  checked_at: "2026-08-09"
  applies_to: "官方文件所述產品入口、賬戶範圍和組織範圍"
  owner: "Skill 維護者"
  next_review: "2026-11-09"
  claim_status: "current"
- claim: "sandbox、審批和安全邊界必須對照當前文件與實際授權的配置進行核查"
