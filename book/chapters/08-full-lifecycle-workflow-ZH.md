...
**状态：** `candidate`。本章介绍一套带证据的工作流和恢复规则，
但比较实验尚未运行。案例材料是项目自有的教学内容，不是一次真实 Codex 运行、
客户委托或生产发布的记录。

## 本章要解决的问题

让模型“开始写”通常不难；把一件有用的工作真正收尾，是另一回事。

任务表面上看似顺利，目标却可能仍然模糊，范围不断扩大，检查跑在错误文件上，
或上一次成功的改动无从确认。模型容量不足可能中断任务，后续提示却可能继续建立在
...
仓库还提供一个可随时丢弃的项目自有案例，把同一套纪律带到非代码交付物：
一张面向首次购房者的虚构房地产概念页。在看截图前先读
...
- 在开始编辑前，写出包含范围、非目标、验收、权限和恢复目标的任务定义；
- 把大请求改写为尽早产生可检查证据的纵向切片（vertical slice），而不是留下多层未完成工作；
- 建立检查点，保留最后一个已知成功状态，让重试有条件，而不是自动发生；
...
|---|---|---|---|
| 所选模型变得不可用，任务停止 | 报告者观察到了容量不足错误和被中断的任务 | 队列语义、服务端原因，或所有账户和版本的行为 | 冻结后续提示；重试前检查 diff、日志和最后接受的检查点 |
...
| 验证被扩大成强制重装 | Agent 可能把“确认它能用”理解为可以改动持久环境 | 报告描述所有 Agent，或重装永远错误 | 分开记录源码改动、测试、安装、重启、部署和线上验证；持久改动前询问 |

本章并不是在教“永不重试”或“永不安装”。它教的是：下一步取决于证据与授权，
而不是等待了多久，或状态标签显得多自信。

## 1. 生命周期：每个阶段都要带着证据

这七个阶段是组织工作的教学模型，不承诺每个 Codex 界面或工作面都会暴露完全相同的
界面状态。

| 阶段 | 它回答的问题 | 常见允许工作 | 出口证据 | 何时停止 |
|---|---|---|---|---|
| 定义 | 想要什么结果、给谁、边界是什么？ | 阅读规则、目标、输入、风险和非目标 | 任务协议与验收说明 | 缺失输入会改变范围、风险、权限或验收 |
| 计划 | 最小且有用的工作顺序是什么？ | 梳理依赖、选择切片、暴露高风险未知项 | 有序计划、切片边界与检查 | 计划只是横向分层，不能独立检查结果 |
...
| 审查 | 声明是否符合证据和请求结果？ | 用新鲜上下文阅读 diff，检查风险与维护成本 | 声明—证据表和开放风险清单 | 声明超出了证据能支持的范围，或授权有歧义 |
...
阶段来填补缺口；把计划写得更长，也不能替代缺少的权限、文件或测试结果。
...
最后一句比前四句严格得多。构建通过很有价值，但不会自动变成运行时、视觉、安全或
用户验收证据。

## 2. 先定义，再行动

定义阶段要把愿望变成有边界的契约。它应短到可以在开工前读完，又具体到足以阻止
好心的 Agent 自行发明范围。
...
任务跨越产品边界时尤其如此。官方安全记录把 sandbox 与审批描述为两种不同的控制，
...
如果界面假设错了，问题可能直到最后才暴露。纵向切片会从输入一路走到证据，
但只选择一个窄结果：
...
例如，“读者能打开一章并找到实验”通常比“迁移整本书的导航”更适合作为第一个
...
检查点是对“当前能确定什么”的记录，不只是时间戳。它应让下一次决策不必依赖之前的聊天
历史。
...
不要把依赖性工作排在尚未确认的检查点之后。现场研究中的用户报告描述过一种危险情况：
...
“继续”不是恢复计划。它没有指出最后接受的状态，不能防止重复副作用，也不能说明
为什么同一动作现在值得再做一次。

## 5. 按声明选择验证

验证的关键是选对检查：选择能够支撑你准备作出的声明的检查。
...
把沉默当作观察，不当作成功信号。启动耗时较长的命令前，先定义预期输出、合理等待时间和
中断路径。等待到期时：
...
产出改动的人或 Agent，往往不是判断它是否完成的最佳人选。用新鲜上下文审查产物，
同时保留原目标与证据清单。
...
交付不是生命周期的终点。若输出包含容易变化的模型、工具、权限、命令或服务事实，
记录其权威 URL、访问日期、范围、负责人和下次复核；若输出是 Skill 或共享工作流，
记录触发条件、排除条件、依赖、测试与回滚。没有人负责这些更新，能力就还不能供团队
可靠使用。

官方 Cloud 文档也说明了为什么这些阶段不能合并：设置、Agent 工作、结果审查和后续跟进
是不同的工作面，需要不同的证据。文档本身只是带日期的产品来源，不证明某个账户或工作区
拥有访问权。
...
这个案例刻意做得很小，用来展示任务协议、Skill、Agent 和行动边界中的字段如何衔接。
它是填好的教学示例，不是一次有运行记录的任务。
...
选择一个 Skill 不会让任务自动完成，也不会自动授予该 Skill 调用其他工具或启动独立工作流
的权限。
...
若 CP2 超出允许范围，先保留 diff 再纠正，并回到 CP0 的恢复来源。若只有一段
...
1. 按技术层逐层完成的横向计划；
2. 从输入走到证据、只取一个窄结果的垂直切片计划。

两套计划必须使用同一验收量表。比较最先暴露的未知项、最先可检查的产物、依赖假设
数量，以及一次刻意中断后的恢复点。
...
学习者通过的条件是：交接中保留检查点、部分 diff、缺失证据、权限边界、恢复路径和
...
把这七个阶段走一遍：先定义读者和结果；计划一处改动；保存原文作为检查点；编辑；比对前后；让新的视角审查；交接给下一位或明天的自己。若需要更多资料或外部动作，先停在 `blocked`，不要为了让流程看起来完成而扩大权限。
...
把这套生命周期应用到一个非代码任务，如研究简报、营销页面或设计交接。对每个阶段写出
