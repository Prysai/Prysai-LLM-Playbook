...
但尚未通过独立学习者前测。不要据此推断某个账户的能力、Cloud 运行结果或模型比较结论。

## 本章要解决的问题

同一个目标可能从桌面应用、CLI、IDE 扩展或网页流程开始；实际执行地点也可能是本机、
Git Worktree 或 Cloud 环境。这些是不同的决策。
...
这句话可能在几个彼此独立的地方出错：目标仓库未必可读；所选模型未必在该工作面可用；
...
这里的“工作面”是工作在哪里执行、改动最终落到哪里；“入口”是人怎样启动和审查这项工作。
...
- 根据上下文、数据边界、副作用、证据和恢复要求，在 `Local`、`Worktree`、`Cloud` 之间选择；
- 区分工作面与桌面端、CLI、IDE 或网页入口；
- 将目标资源访问、模型可用性和工具可用性分别作为独立断言测试；
- 分开记录 Cloud 的 `setup` 与 `agent` 证据，包括网络阶段和秘密信息的生命周期；
- 产出一份 `surface-decision.md`，记录被拒绝和未观察到的选项，而不只记录自己偏好的选项；以及
- 当下一条证据需要的权限超过任务应得范围时，安全地停止。

## 现实入口：阶段才是线索

项目的现场研究收集了 GitHub Issues、Stack Exchange 以及其他公开讨论中的用户报告。
它们是症状报告，不是本地复现、官方根因或保证可用的修复方案。它们的诊断价值在于：
帮助我们看出用户经常把哪些断言混为一谈。

| 公开报告类别 | 报告者观察到什么 | 它**不能**证明什么 | 第一个安全检查 |
|---|---|---|---|
| OAuth 成功，token 交换失败 | 浏览器授权页完成，但客户端没能完成交换 | CLI 会话、目标 host 或仓库可用 | 将授权、回调、交换和第一次无害资源读取记成四个阶段 |
| 自定义 provider 只暴露一个工具 | 配置被接受，但会话中没有 shell、文件或浏览器工具 | 模型或 provider 能执行缺失的动作 | 保存实际工具清单；分别测试注册与调用 |
...
| Cloud setup 看似成功，任务却不能使用秘密或网络 | 依赖安装或出现 setup 标记，但随后 agent 阶段无法访问服务 | setup 网络、秘密和 agent 网络是同一种能力 | 分开记录 setup 日志、agent 日志、网络阶段、秘密生命周期和结果 diff |
...
把 issue #34352 中带日期的公开报告变成目标身份练习。2026-08-12 的研究快照没有记录
...
`Local` 与 `Worktree` 都属于本地执行。Worktree 是 Git 隔离机制，不是通用安全边界；
Cloud 环境是执行边界，但不证明它的 setup、agent 运行时或外部连接已经就绪。
...
链条断裂时，指出断的是哪个阶段。不要把“这个任务”的断言偷偷改写成“产品通常支持”，
以此让声明看起来更强。

## 3. 用五道门选择工作面

按以下顺序评估候选工作面。顺序很重要：它能避免让一个方便的入口或偏好的模型先决定不安全的环境。

### Gate 1：上下文

该工作面能否读取任务所需的精确项目规则、目标文件、版本和验收输入？答案未知时，
不要从仓库名或一次成功登录中推断答案。

### Gate 2：数据边界与隔离

数据应留在当前机器、一次性 worktree 还是获准的远程环境？任务是否含有不能越过边界的
...
这张表提供的是候选项，不是自动授权。即使某个工作面通常合适，任务也可能是 `blocked`。

## 4. 行动前先写决策卡

对于不止是只读说明的任务，先创建 `surface-decision.md`。保留被拒绝的卡片：它们记录为什么
一个看似合理的选项没有被选择。
...
没有运行或未收集证据时，使用 `not_observed`。不要为了填满表格而把缺失证据改成 `yes` 或 `no`。
