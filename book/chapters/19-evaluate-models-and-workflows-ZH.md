...
FP-08（模型/提供方配置不匹配）、FP-09（容量或队列中断）、FP-10（验证命令一直停留在 Working 状态），以及 [FUP-05（长时间无事件后报错，随后自动重试）](../../docs/research/field-problems-follow-up-2026-08-10.md) 都来自公开的用户报告。它们不是官方根因结论、本地复现结果，也不适用于每个账号。
