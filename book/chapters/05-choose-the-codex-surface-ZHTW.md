...
但尚未通過獨立學習者前測。不要據此推斷特定帳戶的能力、Cloud 執行結果或模型比較結論。

## 本章要解決的問題

同一個目標可能從桌面應用程式、CLI、IDE 擴充功能或網頁流程開始；實際執行地點也可能在本機、
Git Worktree 或 Cloud 環境。這些是不同的決策。
...
這句話可能在幾個彼此獨立的地方出錯：目標儲存庫未必可讀；所選模型未必能在該工作面使用；
...
這裡的「工作面」是工作在哪裡執行、改動最後落到哪裡；「入口」是人如何啟動和審查這項工作。
...
- 根據上下文、資料邊界、副作用、證據和復原要求，在 `Local`、`Worktree`、`Cloud` 之間選擇；
- 區分工作面與桌面端、CLI、IDE 或網頁入口；
- 將目標資源存取、模型可用性和工具可用性分別作為獨立宣告來測試；
- 分開記錄 Cloud 的 `setup` 與 `agent` 證據，包括網路階段和秘密資訊的生命週期；
- 產出一份 `surface-decision.md`，記錄被拒絕和未觀察到的選項，而不只記錄自己偏好的選項；以及
- 當下一項證據需要的權限超過任務應有的範圍時，安全地停止。

## 現實入口：階段才是線索

專案的現場研究收集了 GitHub Issues、Stack Exchange 以及其他公開討論中的使用者報告。
它們是症狀報告，不是本地復現、官方根因或保證可用的修復方案。它們的診斷價值在於：
幫助我們看出使用者經常把哪些斷言混為一談。

| 公開報告類別 | 報告者觀察到什麼 | 它**不能**證明什麼 | 第一個安全檢查 |
|---|---|---|---|
| OAuth 成功，token 交換失敗 | 瀏覽器授權頁完成，但用戶端沒能完成交換 | CLI 工作階段、目標 host 或儲存庫可用 | 將授權、回呼、交換和第一次無害資源讀取記成四個階段 |
| 自訂 provider 只暴露一個工具 | 設定被接受，但工作階段中沒有 shell、檔案或瀏覽器工具 | 模型或 provider 能執行缺少的動作 | 儲存實際工具清單；分別測試註冊與呼叫 |
...
| Cloud setup 看似成功，任務卻不能使用秘密或網路 | 相依套件安裝完成或出現 setup 標記，但隨後 agent 階段無法存取服務 | setup 網路、秘密和 agent 網路是同一種能力 | 分開記錄 setup 日誌、agent 日誌、網路階段、秘密生命週期和結果 diff |
...
`Local` 與 `Worktree` 都屬於本地執行。Worktree 是 Git 隔離機制，不是通用安全邊界；
Cloud 環境是執行邊界，但不證明它的 setup、agent 執行時或外部連線已經就緒。
...
鏈條斷裂時，指出斷的是哪個階段。不要把「這個任務」的宣告偷偷改寫成「產品通常支援」，
以此讓宣告看起來更強。

## 3. 用五道門選擇工作面

按以下順序評估候選工作面。順序很重要：它能避免讓方便的入口或偏好的模型先決定不安全的環境。

### Gate 1：上下文

該工作面能否讀取任務所需的精確專案規則、目標檔案、版本和驗收輸入？答案未知時，
不要從倉庫名或一次成功登入中推斷答案。

### Gate 2：資料邊界與隔離

資料應留在目前的電腦、一次性 worktree 還是獲准的遠端環境？任務是否含有不能越過邊界的
...
這張表提供的是候選項，不是自動授權。即使某個工作面通常合適，任務也可能是 `blocked`。
...
沒有執行或未收集證據時，使用 `not_observed`。不要為了填滿表格，把缺失證據改成 `yes` 或 `no`。
