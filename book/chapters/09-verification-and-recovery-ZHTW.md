...
**狀態：** candidate。**實驗狀態：** not_run。本章教授如何讓完成宣告與證據相符，
以及工作流程變得不確定時如何復原。公開報告和範例都是教學輸入，不是本機重現、
官方根因、客戶工作或正式環境證據。

## 本章要解決的問題

Agent 可以為錯誤、超出範圍、從未執行，或在錯誤環境中檢查的結果寫出極具說服力的
完成摘要。可靠的回應不是盲信，也不是永遠懷疑；而是把摘要拆成宣告，並為每條宣告
分配在其宣告範圍內足以支持它的最小證據。
...
這些報告有用，是因為它們暴露了工作流程中可觀察的斷點；但它們不建立普遍服務原因、
所有帳戶的修復方式或本機重現。用它們練習一個關鍵區分：重新取得一次執行的控制權，
不等於證明預期結果正確。

| 報告症狀 | 報告支持什麼 | 它不能證明什麼 | 第一個安全回應 |
|---|---|---|---|
| 所選模型不可用，任務停止 | 有報告者看到了容量錯誤和中斷任務 | 佇列語意、伺服器端原因，或每個帳戶／版本的行為 | 凍結後續提示；重試前檢查 diff、日誌和最後接受的檢查點 |
| 格式化或驗證持續 Working | 該次執行沒有完成訊號 | 普遍死鎖、確切子程序或根因 | 設定有界等待，儲存輸出和程序狀態，只按復原規則中斷 |
| 工作階段可用但預期工具缺失 | 工作階段與工具庫存不符合任務預期 | 每個提供商／版本暴露相同工具集 | 記錄實際工具清單；在檔案、瀏覽器或外部服務動作前停止 |
| 驗證變成強制重灌 | 「檢查結果」可能被理解成獲准改動持久環境 | 重灌永遠錯誤，或報告涵蓋所有 Agent | 分開記錄原始碼、測試、安裝、重啟、部署和線上驗證 |

下一步應由證據和授權決定，而不是由等待時間或狀態標籤的自信程度決定。

## 三份 Windows 報告：訊號不是證據

下列案例來自 2026-08-12 查閱的公開 GitHub 報告。它們是教學輸入，
不是本機重現、官方診斷或普遍的 Windows 行為。

| 公開症狀 | 它可教什麼 | 第一個有界檢查 | 停在何處之前 |
|---|---|---|---|
| CLI 輸出超出終端視窗尺寸後無法從捲動緩衝區找回（[#35335](https://github.com/openai/codex/issues/35335)） | 視窗呈現不是持久證據 | 將回應儲存或重新產生到具名檔案；記錄 CLI、終端和提示範圍 | 把缺失的視窗內容宣告為 Agent 或儲存庫資料遺失 |
| 非 BMP 字元貼上到 TUI 編輯框後消失（[#37578](https://github.com/openai/codex/issues/37578)） | 編輯框回顯不等於輸入完整性 | 在傳送有後果的請求前，用無害 fixture 比較預期與實際接收的字串 | 編輯、提交或傳送未被保留的輸入 |
| 很長的檢查點引用讓 Windows Git 報 bad ref 或 Filename too long（[#37559](https://github.com/openai/codex/issues/37559)） | 內部 Agent 狀態不等於一般專案狀態 | 在獲批的診斷範圍內記錄 git status、git show-ref、git fsck --full、git worktree list 與精確 ref 路徑 | 未備份、未授權就刪除 .git 材料、修改 Git 設定、抓取或修復引用 |

詳見[Windows 輸入與證據現場問題](../evidence-library-ZHTW.md#source-notes)。
實用規則是：**重試前先捕獲最小的持久產物**——輸出檔案、收到輸入的比較、diff、雜湊、
...
若必要輸出被隱藏或缺失，只保留已獲授權的退出碼、事件、diff、產物、雜湊或回讀證據；
把審計宣告標為 unverified，並寫明缺失通道。不要只為找回呈現輸出而重跑有後果的動作、
...
不要用一次 diff 證明測試通過，也不要用登入頁面證明權杖交換或外部動作成功。證據缺失時，
...
最後一句嚴格強於前四句。構建通過有價值，但不會自動成為執行時、視覺、安全或使用者驗收
證據。
...
不要用「再跑一次」、「多給權限」或「讓模型更努力想」替代診斷。

### 能力鏈：每一層成功都需要自己的證明

公開報告反覆出現一個誤導順序：工具名稱出現在清單、網頁可讀、提供商接受設定，
...
工具名可見而唯讀發現失敗，斷點就是「工具已註冊且可發現」；動作返回成功而目標狀態不變，
斷點就是「預期副作用已確認」。不要越過斷點做更高風險的動作，也不要用後來的幸運成功
倒填早期證據。
...
2. 用已授權的方式重新取得控制，再檢查程序、工作樹、目標狀態和最後檢查點；
...
復原可以重新取得控制，卻不升級完成狀態。例如，中斷一個掛起程序並儲存 diff，可能形成有用
...
不是本機重現或官方根因。參見[網頁現場研究](../evidence-library-ZHTW.md#source-notes)
...
準備去識別化的完成摘要、diff、測試輸出、來源連結和一項刻意缺失的證據。不要連線正式環境服務
...
在可丟棄副本中開始一項小而可逆的改動。跑檢查前寫一份稱「完成」且「全部測試通過」的
