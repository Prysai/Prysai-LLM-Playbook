...
**상태:** `candidate`. **실험:** `draft / not_run`. 이 장은 개발 작업의 전체 흐름을 다루며, 현장 보고는 로컬 재현이나 모든 버전의 원인 확인이 아닙니다.

## 이 장에서 해결하는 문제

개발 작업은 요구사항, 설계 선택, 테스트 설계, 런타임 관찰, rollback 방법을 정하기 전에 코딩을 시작하게 만듭니다. patch가 build되고 unit test가 모두 통과해도 사용자 경로, 오류 처리, 의존성 버전, 배포, 복구가 제대로 된다는 뜻은 아닙니다.

> build가 성공한 것, unit test와 integration test가 통과한 것, 런타임에서 제대로 동작하는 것, 사용자가 받아들이는 것, 프로덕션에 준비된 것은 서로 다른 주장입니다.

엔지니어링 Skill은 증거를 남기며 진행하는 흐름이어야 합니다. 각 단계에는 진입 조건, 가장 작은 slice, 실패 경로, 단계가 끝났음을 보여 주는 증거가 있습니다.
...
## 구현 전에 요구사항과 검수 기준 정하기

“export 기능을 추가”라면 형식, 데이터 범위, 권한, 일부만 만들어진 파일 처리, 덮어쓰기 정책, 최종 검수 기준을 묻습니다. 사용자 행동, 입력 제약, 성공·오류 출력, 경계, 비목표, 성능·보안 제약, 관찰 신호, 검수 방법을 적습니다. Skill은 결정을 조용한 기본값으로 바꾸지 못합니다.
...
build 증거는 compile할 수 있다는 뜻이고, test 증거는 지정한 assertion이 통과했다는 뜻입니다. 런타임 증거에는 start command, version, 환경 값, 실제 입력, response 또는 화면, log, 오류 경로가 필요합니다. 프로덕션 준비에는 security, performance, migration, monitoring, rollback, 사용자 검수도 더 필요합니다.
