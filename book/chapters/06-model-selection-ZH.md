...
**状态：** `candidate`。下文的比较协议已经写明，也明确了来源边界，但本仓库尚未运行固定任务集。模型性能、成本、延迟、容量、稳定性和任何总体排名仍为 `not_run`。

## 本章要解决的问题

谈模型选择时，人们常用一句口号替代判断：“用最好的模型。”真实工作需要问得更具体：

> 针对这项任务，在这个工作面、这个提供商、这组上下文与工具、这条权限边界、这个时间预算和验收标准下，哪个候选项能达到最低要求？我们是否已有足够证据扩大试验？

若候选模型在所选工作面不可用，或两次运行使用了不同输入、工具、权限或推理设置，就无法进行公平的模型比较。漂亮演示至多说明某个配置曾产生一个结果，不能据此建立通用排名或总体性价比结论。
...
- 运行低风险的三任务快速比较，而不为“救回”某一个候选项而改变条件；
- 将容量不足、提供商不匹配和长时间等待失败保存为证据；以及
- 说明实验能证明什么、不能证明什么，以及何时该停止。

## 现实问题入口：模型选择会在日常情境中失败

项目的 [Codex 现场研究](../evidence-library-ZH.md#source-notes) 汇集公开 GitHub Issue 和其他公开讨论。这些记录描述的是症状，不是官方诊断或本地复现；它们的价值在于暴露模型选择出错时人们常作出的假设。
...
| 所选模型达到容量上限 | 任务在得到完整结果前停止，后续提示可能面对部分状态 | 模型质量低，或重试就代表第一次已经完成 | 保存检查点、diff、日志和测试；继续前先分类该状态 |
| Windows 命令持续显示 `Working` | 界面显示仍在活动，却没有可验证输出 | 格式化器、Agent 或模型仍在进行有用工作 | 采用超时/停止规则，安全中断，检查 worktree，只重跑有界检查 |

原始链接、日期、版本、证据等级和不确定性说明见[模型选择研究记录](../evidence-library-ZH.md#source-notes)。本项目没有执行这些报告中出现的命令或替代方案。
...
3. **官方确认：** 维护者回复、官方文档、发行说明或其他来自第一方的证据。
4. **Playbook 证据：** 本项目实际复现了什么。

上面三个例子可能已有前两类标签，但项目没有本地复现，也没有官方根因确认，不能把它们升级为保证有效的修复方案。因此正确做法是保存证据、缩小下一步检查，而不是承诺一个“神奇设置”。
...
每一个箭头对应不同断言。官方页面可以描述某模型，它仍可能不对某个账户开放；它也可能出现在选择器中，却在请求到达提供商时失败。一次成功的文本回复同样不能证明任务所需的文件、终端、浏览器或连接器都可用。
...
这些是产品描述，不是 Playbook 的基准测试结果。官方页面同样提醒：更高推理强度可改善复杂工作，但会花更久、使用更多 token。先用满足验收标准的最低强度；只有任务确实需要更多规划、分析或检查时才提高，并将该设置记入运行记录。
...
一个候选项通过提取任务，仍可能不适合多文件修复或高风险证据审查。验收标准必须匹配任务类别。
...
冻结 `task_set_version: three-task-smoke-v1`、两张候选卡、一份验收标准、原始输出位置、日志位置，以及针对不可用、容量中断、权限不匹配、输入漂移或工具版本漂移的停止条件。
...
3. 按相同任务顺序、相同输入和相同验收标准运行候选项 A 与 B。
...
面对容量或长等待失败，正确做法不是“不断点击直到成功”。应该保存最后已知状态，先判断任务是完成、部分完成还是未知，再选择有界恢复。新对话可以作为恢复工作面，却不会继承旧对话的完成证明。
...
冻结新的任务集版本和领域验收标准。不要把模型选择或三任务结果直接复制到新领域；说明哪些结论仍局限于该任务，哪些主张必须放弃。

## 本章证据

预期交付物是两张候选卡、冻结的任务集与验收标准、初始原始运行及任何受控返工、一张比较表、类型化错误记录，以及扩大/停止决定。在这些记录存在前，本章实验必须保持 `not_run`；官方定位和一次演示都不能替代评测证据。
