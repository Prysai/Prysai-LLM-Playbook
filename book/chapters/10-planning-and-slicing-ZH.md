...
实验是一项可以在本地完成的低风险练习；本次编辑没有代替学习者运行它。

## 本章要解决的问题

“把整个功能做完”“把网站做完”或“让工作流达到生产可用”听起来像目标，但还不能直接执行。
这些说法把好几种不确定性都藏在了一句话里：

- 用户首先应该看到什么；
- 下一步要有意义，哪个假设必须先成立；
- 哪些文件、服务或人员属于允许的范围；
- 怎样区分“做到一半”和“可以交付”；以及
- 命令卡住、模型不可用，或者需求做到一半发生变化时该怎么办。

Agent 可以写出很长、看起来很有条理的计划，却把第一份有意义的证据一直推到最后。它也可能完成
一个技术上很小的任务，却留下无法接手的结果：文件改了，但没人知道该运行什么、哪些事情还没有
得到证明，也不知道重试会不会再次产生副作用。

所以，真正有用的工作单位不是待办清单中的一行，而是一条有边界、能被观察的交付切片：用一次小改动
产生结果，留下证据，说明限制，并为下一条切片留下安全的衔接点。

![教学卡：生命周期检查点把大目标拆成带证据的交付节点](../../assets/teaching/locales/zh/lifecycle-checkpoints.svg)
...
这一区分很重要。计划不是 Agent 一定会完成的承诺，而是一个控制面，让人可以暂停、检查，并在不靠猜测的
情况下做出下一步决定。
...
先写一句用户、审查者或下游系统能够直接观察到的话：

> 新贡献者能阅读一页内容，在无需网络的情况下运行一项本地检查，并看到明确的通过或失败结果。

这句话比“搭建文档流水线”更有用，因为它明确了计划要保护的结果。再把它改写成一张结果卡：

~~~text
结果：现在有人能做什么，或能检查什么
输入：文件、数据、假设和前置条件
允许动作：准确的操作范围和允许的变更类型
非目标：这次故意推迟的、虽然诱人但不在范围内的工作
证据：文件、diff、命令输出、测试、截图或审查记录
风险：秘密信息、外部调用、持久化、删除或不可逆的修改
~~~

如果必须等未来的所有功能都完成后才能看到结果，就先把结果拆小。“API、界面、迁移、分析和
部署全部完成”是发布里程碑，不是第一条切片。

### 2. 把依赖画成事实，不是愿望清单

依赖的意思是：“下一步动作要产生有意义的结果，这件事必须先成立。”它不只是
“这项工作在工单里排在前面”。

对每个候选项都问四个问题：

1. **依赖于：** 哪个准确的状态或证据必须先存在？
2. **提供给：** 下一条切片会拿到哪个文件、字段、命令结果或决定？
3. **受阻于：** 哪个缺失的输入不能安全猜测？
4. **依赖检查：** 哪项成本最低的只读检查可以确认前置条件已经存在？
...
箭头表示的是证据和交接接口，不是 Agent 隐藏的记忆。如果依赖还没有检查，就标为
assumed，不要默默把它当成已经成立。
...
| 垂直 | 证明从输入到可观察结果的一条细路径 | 第一条切片被做得太大，悄悄变成“整个功能” | 需要早期反馈、可逆实验或交接 |

<mark class="highlight-text highlight-cyan">垂直切片不是“后端的一小块”。</mark>
它只跨过展示一个结果所必需的最少边界。它可以同时涉及文档、脚本和测试，但应排除无关的润色、
额外角色、生产数据和未来的抽象层。

当主要问题还没有答案时，使用**探针**。探针是低成本、只读或可逆的调查，例如检查依赖是否存在、
渲染一个样本，或确认一次无害的工具调用能到达目标范围。不要把探针包装成产品工作；它的产物应该是
一个决定：继续、缩小范围，还是停止。
...
这些字段可以防止一种常见的偷换：把“某人能够完成 X”换成“Agent 编辑了 Y 和 Z 文件”。
文件是实现证据，但不会自动变成交付证据。
...
预算不是 token 用量预测，而是副作用的边界。如果切片需要预算外文件、新依赖、生产凭据或
另一个仓库，就应该停下来更新计划。“验证需要”不会自动授予安装或部署权限。
...
检查点必须保存在 Agent 的短期对话状态之外。一份小 Markdown 文件、Issue 记录或已批准的
任务记录就够了。不要把秘密信息、Cookie、token 或私有凭据写进去。

### 6. 判断切片是否真的足够小

如果标题包含多个独立的用户结果、把实现／迁移／发布混在一起、需要多种验收权威，或者可能在多个层面失败却
没有第一处断点，切片多半太大。如果第一份有用证据只能到最后才出现，就应该拆开。

如果切片只创建了一个无法阅读、运行、审查或连接到可观察行为的孤立文件，或者只是后续切片所需的
内部重构，它多半又太小。除非这个重构本身就是刻意设计的探针，否则应与最近的可观察切片合并。
...
如果不能，切片就需要更清楚的交接接口，或者更小的结果。

<a id="core-evidence-bearing-slice-end"></a>

## 可复用的规划提示词

把下面的内容当作起点，替换示例值；不要原封不动地粘贴到生产任务中。

~~~text
目标
交付一条垂直切片：[一名具名用户或审查者可观察的结果]。
...
提示词本身不会让 Agent 变得可靠。它的价值在于把对话式请求变成另一人可以审查的契约。

## 小实验：三种计划，一条安全切片

本实验不使用网络、安装、凭据、提交、推送、部署或生产数据。请使用临时目录，不要使用包含他人
修改的工作树。

### 准备

创建一个临时文件夹，里面的 README.md 只包含：
...
验收目标故意设得很小：读者必须能看到“改了什么”和“如何验证”两个小节。本地检查是只读搜索。
在 PowerShell 中：
...
在其他 Shell 中使用等效的无网络检查：任一标题缺失时返回非零状态。记录你实际运行的命令。

### 任务

围绕同一个目标，各写一份不超过7项的计划：

1. 按写作、工具、审查和发布分层的横向计划；
2. 按预计会触及的文件顺序排列的计划；
3. 先做最小可阅读、可检查页面的垂直计划。

每一项都必须有结果、依赖、证据和停止条件。随后只执行垂直计划的第一项：添加两个标题，
并在每个标题下写一句真实的话。不要添加样式、链接、构建系统或未来的章节。

编辑前后记录目标路径和 diff，并运行一次本地检查。你的证据记录应该让一个陌生人回答：
...
预期产物不是“一份漂亮的 README”，而是一项小而可审查的变更，以及足以判断下一条切片是否安全的证据。

## 故意失败：让断点可见

现在制造一次受控失败：从 README.md 删除“## How to verify”，或让检查读取一个不存在的
路径。运行检查一次并保存非零输出。不要为了让错误消失而添加依赖、改变权限或重写整页。
...
恢复后写下改了什么，以及失败**没有**证明什么。一次失败的检查只能证明该检查没有在这个输入上通过；
它不能证明整个仓库或 Agent 已损坏，也不能证明应该进行更大范围的重写。

## 真实工作中的恢复与停止条件

长任务中断后，不要无条件地发送“继续”。按以下顺序处理：

1. **冻结新的副作用。** 状态不明时，不要排队另一项写入、外部请求、安装、删除或部署。
2. **检查最后已知状态。** 读取检查点、git status、相关 diff、最后的命令输出和真实目标路径。
3. **给断点分类。** 标为输入缺失、范围漂移、验证失败、基础设施／超时、权限或未知。
4. **选择一项有边界的动作。** 优先做只读探针，或对最小切片进行幂等重试；一次只改变一个变量。
5. **更新检查点。** 记录新证据，以及下一条切片是否仍然有效。
6. **停止或交接。** 如果原因仍然未知、重试预算已用完或目标发生变化，就交付一份受阻交接，不要继续堆编辑。

只有在部分状态不安全、容易误导或维护成本太高时才回滚。回滚前保留失败证据；不要为了得到
看起来干净的工作树而丢弃用户已有的修改。如果部分产物安全且有信息价值，就保留它，并标为 partial 或
unverified。
...
这条规则在上下文压缩、模型容量错误、长时间运行的命令、工作树发生变化，或提示词要求另一个结果时尤其
重要。对话摘要可能保留意图，却不一定保留准确的文件状态、进程状态或外部副作用。

## 预期产物：另一个人可以执行的交接

即使结果受阻，一条完整的切片也会留下这些产物：
...
交接是切片的一部分，不是“真正的工作”结束后才补的文书。没有它，下一位就得从漫长的聊天记录重建
状态，还可能重复已经发生的动作。

## 反思

离开本章前，写下一项目前仍然大到无法交给别人的任务。哪一条第一切片能最早产生有意义的
证据？哪项动作、输入或决定必须明确排除在范围外？如果在这条切片后中断，什么记录能让别人
不靠猜测继续？

## 迁移任务

从另一个领域选择一项真实但低风险的任务，并写3到7张切片卡：

- **工程：** 添加一个只读端点和一个显示固定夹具的界面。第一条切片排除认证、分析和部署。
- **研究：** 用来源表、不确定性列和边界清楚的结论回答一个窄问题。在来源核查前，不要把搜索
  结果称为已验证事实。
- **营销：** 把一份已批准的产品上下文改成面向特定受众的草稿，再设计一项可审查的实验。排除发布和
  实时受众数据。
- **Skill 设计：** 写清一个触发条件、输入结构、允许动作、预期输出、失败路径和复核日期。在规划
  练习中，不要安装或调用未经审查的外部 Skill。

让一名同事，或一个全新的 Agent 上下文，在没有口头解释的情况下审查这些卡。他们要找出
一个隐藏假设和一个缺失的验收信号。直接修改切片卡，不要依赖审查者的记忆。

## 验收清单

- [ ] 我能用一句话说出第一项用户或审查者可以观察到的结果。
- [ ] 我能区分横向计划、文件顺序计划、垂直切片和发现探针。
- [ ] 每条切片都有 depends_on、provides_to、依赖检查和明确写出的非目标。
- [ ] 每条切片都有强于“Agent 说它完成了”的证据规则。
- [ ] 我在任何不可逆动作前都设定了文件／动作／变更预算和检查点。
- [ ] 我能识别计划何时因为推迟第一份有意义的证据而变得太大。
- [ ] 我能识别计划何时因为没有可观察行为或可审查接口而变得太小。
- [ ] 我能在超时、范围变化、缺少授权或目标未知时停止，而不自动堆叠编辑。
- [ ] 我能通过重新读取状态、一次只改变一个变量，从部分结果中恢复。
- [ ] 我能在交接中区分 done、partial、blocked、unverified、not_proven 和 next。
- [ ] 在没有本地运行记录和审查证据前，我能说明本章实验仍为 not_run。

## 来源与更新边界

本章的规划方法是项目自有的综合。它刻意区分用户报告、官方产品事实、项目推断和本地复现。
...
| 外部检查点、垂直切片和完整交接是高价值课程补充 | [内容价值升级计划 P2](../evidence-library-ZH.md#source-notes)，访问于 2026-08-11 | 项目规划建议；拟议实验仍为 not_run | curriculum-maintainer / 2026-09-11 |
...
不要复制链接报告中的文字、截图、代码或 Skill 指令。本章只把这些报告中的发现作为研究输入，
再用原创文字记录项目的规划方法。实验命令刻意限制在本地且不联网；它们不能据此推出每个 Codex
工作面、操作系统、模型或提供商的行为结论。

## 练习恢复边界

当任务在中断、上下文丢失、分支变化或权限状态变化后恢复时，进入
