...
제안, 승인, 실행, 관찰된 효과, 수용 판정을 구분하고, 입력(input), 권한(authority), 증거(evidence), 예산(budget)에 대한 중단 조건을 시작 전에 정하며, 일어났을 수 있는 쓰기 작업을 다음 사람이 그대로 반복하지 않도록 인계 기록(handoff)을 작성할 수 있습니다. 이 연습은 일반적인 Agent나 호스트 동작을 증명하지 않습니다.
...
| 효과 | 되읽기, diff, hash 또는 외부 영수증 | 사용자 규칙을 충족한다는 것 |
