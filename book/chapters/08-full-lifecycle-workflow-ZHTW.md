...
| 交付 | 另一個人能否使用並檢查結果？ | 交接檔案、日誌、限制和下一步 | 交付摘要與精確產物路徑 | 狀態會被誇大成已提交、已發布或已上線 |
...
| 已發布並上線 | 部署記錄加交付後請求／檢查 | 不證明每個快取、路由、裝置和使用者路徑都正確 |
...
初學者不必從網站、程式碼或發布開始。選一個自己能檢查的短文字、一個本機 README，或一組已經允許使用的公開來源。目標不是讓模型「做很多」，而是完成一次從定義到交接都看得見的閉環。

```text
結果：讓一段 120 字以內的說明讓新讀者能找到第一步。
輸入：原文、讀者是誰、一個已知問題。
允許：只讀原文；提出計劃；確認後只編輯該文字。
不允許：連網、登入、安裝、傳送、發布或改動其他檔案。
