...
**狀態：** `candidate`。本章介紹一套帶著證據運作的工作流程與復原規則，
但比較實驗尚未執行。案例材料是專案自有的教學內容，不是一次真實 Codex 執行、
客戶委託或正式發佈的記錄。

## 本章要解決的問題

讓模型「開始寫」通常不難；要把一件有用的工作真正收尾，則是另一回事。

任務表面上看似順利，目標卻可能仍然模糊，範圍不斷擴大，檢查跑在錯誤的檔案上，
或上一次成功的改動無從確認。模型容量不足可能中斷任務，後續提示卻可能繼續建立在
半成品狀態上；終端可能停在 `Working`，卻沒有命令完成的證據；瀏覽器
可以報告登入成功，但用戶端仍可能在下一步權杖交換時失敗。
...
儲存庫還提供一個可以隨時丟棄的專案自有案例，把同一套紀律帶到非程式碼交付物：
一張面向首次購屋者的虛構房地產概念頁。在看截圖前先讀
[案例記錄](../evidence-library-ZHTW.md#source-notes)。
其中標明合成輸入、本機渲染命令、記錄的視窗尺寸，以及這張圖不能支持的宣告。

[![合成首次購房者指南的一次本機渲染](../../assets/cases/product-context-real-estate-thumbnail.png)](../../assets/cases/product-context-real-estate-desktop.png)

截圖只是指定視窗尺寸下的一次本機渲染證據。它不能證明 Product Context Skill 曾獨立
執行、房源真實存在，或頁面提高了信任、諮詢、轉換或銷售。
...
- 在開始編輯前，寫出包含範圍、非目標、驗收、權限和復原目標的任務定義；
- 把大請求改寫為能儘早產生可檢查證據的垂直切片（vertical slice），而不是留下多層未完成工作；
- 建立檢查點，保留最後一個已知的成功狀態，讓重試有條件而不是自動發生；
- 區分構建、執行時、視覺、來源、安全和使用者驗收證據；以及
- 寫出誠實的交接，說明發生了什麼、沒有發生什麼、下一位審查者還必須檢查什麼。

## 現實問題：工作流程可能在兩個介面之間失敗

專案的 [Codex 現場研究](../evidence-library-ZHTW.md#source-notes) 記錄公開使用者報告。
這些報告是有用的症狀，不是官方根因分析，也不是本機重現。

| 報告的症狀 | 報告能夠支持什麼 | 它**不能**證明什麼 | 第一個安全回應 |
|---|---|---|---|
| 所選模型變得不可用，任務停止 | 報告者觀察到了容量不足錯誤和被中斷的任務 | 佇列語意、伺服器端原因，或所有帳戶和版本的行為 | 凍結後續提示；重試前檢查 diff、日誌和最後接受的檢查點 |
| 格式化或驗證任務長時間停在 `Working` | 報告者在該次執行中沒有看到完成訊號 | 普遍死鎖、確切子程序或根因 | 設定有界等待；儲存輸出和程序狀態；僅按任務復原規則中斷 |
| 瀏覽器稱認證成功，用戶端稍後失敗 | 認證有多個可觀察階段 | 瀏覽器成功、網路可達或頁面提示證明用戶端已就緒 | 將回呼、權杖交換和第一次成功的用戶端請求分別記錄 |
| 驗證被擴大成強制重灌 | Agent 可能把「確認它能用」理解為可以改動持久環境 | 報告描述所有 Agent，或重灌永遠錯誤 | 分開記錄原始碼改動、測試、安裝、重啟、部署和線上驗證；持久改動前先詢問 |

本章並不是在教「永不重試」或「永不安裝」。它教的是：下一步取決於證據與授權，
而不是等待了多久，或狀態標籤顯得多自信。

## 1. 生命週期：每個階段都要帶著證據

這七個階段是組織工作的教學模型，不承諾每個 Codex 介面或工作面都會顯示完全相同的
介面狀態。

| 階段 | 它回答的問題 | 常見允許工作 | 出口證據 | 何時停止 |
|---|---|---|---|---|
| 定義 | 想要什麼結果、給誰、邊界是什麼？ | 閱讀規則、目標、輸入、風險和非目標 | 任務協議與驗收說明 | 缺失輸入會改變範圍、風險、權限或驗收 |
| 計劃 | 最小且有用的工作順序是什麼？ | 梳理依賴、選擇切片、暴露高風險未知項 | 有序計劃、切片邊界與檢查 | 計劃只是橫向分層，不能獨立檢查結果 |
| 構建 | 在允許範圍內實際改了什麼？ | 做一次有界改動並保留檢查點 | diff、變更檔案清單與檢查點記錄 | diff 超出範圍，或復原目標不清楚 |
| 驗證 | 結果在關鍵檢查下是否表現正確？ | 跑聚焦測試、構建、執行時、視覺或來源檢查 | 命令、退出碼、輸出、環境與限制 | 命令掛起、測試目標錯誤，或缺少證據 |
| 審查 | 宣告是否符合證據和請求結果？ | 用新鮮上下文閱讀 diff，檢查風險與維護成本 | 宣告—證據表和開放風險清單 | 宣告超出證據能支持的範圍，或授權有歧義 |
| 交付 | 另一個人能否使用並檢查結果？ | 交接檔案、日誌、限制和下一步 | 交付摘要與精確產物路徑 | 狀態會被誇大成已提交、已發佈或已上線 |
| 維護 | 什麼需要觀察、更新或回滾？ | 記錄負責人、源版本、下次複核和回滾 | 維護記錄與複核觸發器 | 沒有人負責更新，或回滾無法演練 |

出口條件缺失時，寫 `blocked` 或 `unverified`。不要靠再加一個
階段來填補缺口；把計劃寫得更長，也不能替代缺少的權限、檔案或測試結果。
...
| 「頁面看起來正確。」 | 指定視窗尺寸的渲染檢查和視覺驗收標準 |
| 「功能已經交付。」 | 儲存庫或部署狀態、發佈記錄和交付後檢查 |

最後一句比前四句嚴格得多。構建通過很有價值，但不會自動變成執行時、視覺、安全或
使用者驗收證據。

## 2. 先定義，再行動

定義階段要把願望變成有邊界的契約。它應短到可以在開工前讀完，又具體到足以阻止
好心的 Agent 自行發明範圍。
...
1. `non_goals` 防止任務意外擴張。「驗證頁面」並不暗含重灌瀏覽器、
   改系統策略或發佈結果。
2. `rollback` 必須指向真實的復原來源。雜湊只能說明檔案變了，不能
   自己復原舊內容。

### 最小權限規則

從唯讀檢查開始；只為已命名目標增加寫入權限；只有任務確實需要並且範圍已獲授權時，
才增加網路、認證、安裝、重啟、部署或外部訊息的權限。

任務跨越產品邊界時尤其如此。官方安全記錄把 sandbox 與審批描述為兩種不同的控制，
也將有副作用的 Connector 或 MCP 動作置於審批邊界。因此工作流程既要記錄技術能力，
...
如果介面假設錯了，問題可能直到最後才暴露。垂直切片會從輸入一路走到證據，
但只選擇一個窄結果：
...
例如，「讀者能開啟一章並找到實驗」通常比「遷移整本書的導覽」更適合作為第一個
...
| 證據 | diff、驗證器輸出、本機連結輸出和限定檔案審查 |
| 未證明 | 翻譯品質、讀者理解、瀏覽器部署或執行時 Skill 行為 |
...
檢查點是「目前能確定什麼」的記錄，不只是時間戳。它應讓下一次決策不必依賴之前的聊天
歷史。
...
不要把依賴性工作排在尚未確認的檢查點之後。現場研究中的使用者報告描述過一種危險情況：
...
「繼續」不是復原計劃。它沒有指出最後接受的狀態，不能防止重複副作用，也不能說明
為什麼同一動作現在值得再做一次。

## 5. 按宣告選擇驗證

驗證的關鍵是選對檢查：選擇能夠支撐你準備作出的宣告的檢查。

| 宣告 | 能支撐它的檢查 | 該檢查的邊界 |
|---|---|---|
| 預期檔案改變 | 指定路徑的 diff | 不證明改動正確 |
| 語法或構建有效 | 聚焦驗證器或構建命令 | 不證明執行時行為 |
| 功能在一個環境表現正常 | 固定輸入的執行時檢查 | 不泛化到每個帳戶、系統或提供商 |
| 頁面按意圖渲染 | 指定視窗尺寸的瀏覽器或視覺檢查 | 不證明使用者需求、無障礙完整性或正式部署 |
| 外部事實仍新鮮 | 有日期、範圍和下次複核的權威來源 | 不證明本帳戶有存取權，或本工作階段的設定正確 |
| 已發佈並上線 | 部署記錄加交付後請求／檢查 | 不證明每個快取、路由、裝置和使用者路徑都正確 |
...
把沉默當作觀察，不當作成功訊號。啟動耗時較長的命令前，先定義預期輸出、合理的等待時間和
中斷路徑。等待到期時：
...
這種不確定性正是復原規則必須依賴證據、不能依賴猜測根因的原因。

## 6. 不依賴執行者來審查

產出改動的人或 Agent，往往不是判斷它是否完成的最佳人選。用新鮮上下文審查產物，
同時保留原目標與證據清單。
...
交付不是生命週期的終點。若輸出包含容易變動的模型、工具、權限、命令或服務事實，
記錄其權威 URL、查閱日期、範圍、負責人和下次複核；若輸出是 Skill 或共享工作流程，
記錄觸發條件、排除條件、依賴、測試與回滾。沒有人負責這些更新，能力就還不能供團隊
可靠使用。

官方 Cloud 文件也說明了為什麼這些階段不能合併：設定、Agent 工作、結果審查和後續跟進
是不同的工作面，需要不同的證據。文件本身只是帶日期的產品來源，不證明某個帳戶或工作區
擁有存取權。
...
**安全的第一個回應：** 凍結依賴提示，儲存目前的 diff 和日誌，確定最後接受的檢查點，
檢查預期檔案或產物是否部分完成；之後選擇一次有界重試、備用工作面或交接。

**不得宣告：** 排隊的任務已經完成、模型是唯一原因，或反覆傳送「繼續」已補回缺失證據。
...
**不得宣告：** 「還在執行」等於「已通過」，或沒有可見錯誤就等於子命令已完成。
...
**不得宣告：** 瀏覽器成功證明用戶端認證、帳戶權益、連接器審批或 MCP 工具可用。

### 驗證要求修改持久環境

**觀察到的症狀：** Agent 提議重灌、重啟或改動本機環境來讓檢查通過。
...
| Workflow Orchestrator | 作為階段記錄使用 | 跟蹤依賴和檢查點；不擴大權限 |
| Evidence Review | 使用 | 把「連結檢查通過」和「只改一個檔案」對應回證據 |
| Research | 暫不使用 | 固定缺陷清單不需要新外部事實 |
| 瀏覽器、連接器、GitHub 寫操作 | 不使用 | 對本機 Markdown 審查沒有增加價值 |

選擇一個 Skill 不會讓任務自動完成，也不會自動授予該 Skill 呼叫其他工具或啟動獨立工作流程
的權限。
...
| 計劃 | 排序兩三個本機編輯 | 計劃、依賴順序、假設 |
| 構建 | 只編輯 docs/guide.md | diff、檢查點、變更檔案清單 |
| 驗證 | 跑已有本機檢查 | 命令、退出碼、輸出、限制 |
| 審查 | 對照目標閱讀 diff | 審查筆記和宣告—證據表 |
| 交付 | 準備本機審查包 | 寫明是否發生 commit/push 的摘要 |

任一階段缺少出口證據時，標為 `blocked` 或 `unverified`；
不要因為計劃中寫了下一階段，就假定可以繼續。
...
若 CP2 超出允許範圍，先保留 diff 再糾正，並回到 CP0 的復原來源。若只有一個段落
錯誤，就修這一段並重跑相關檢查；不要先確認精確目標和復原來源就使用寬泛還原命令。
...
選擇低風險的功能或文件交付，使用可丟棄副本或明確隔離的分支。準備去識別化輸入、固定
驗收量表和起始狀態記錄。不要發佈、推送、刪除或改動生產資源。
...
1. 按技術層逐層完成的橫向計劃；
2. 從輸入走到證據、只取一個窄結果的垂直切片計劃。

兩套計劃必須使用同一驗收量表。比較最先暴露的未知項、最先可檢查的產物、依賴假設
數量，以及一次刻意中斷後的復原點。

### 證據

儲存兩份計劃、依賴草圖、切片的進入與退出條件、實際 diff、驗證輸出和檢查點筆記。
只有至少一個切片產生獨立證據，並且學習者能說出仍未完成的工作，實驗才算通過。
...
- 模型變得不可用；
- 驗證命令在定義的等待時間內沒有輸出；或
- 建議的復原需要安裝、重啟、網路呼叫，或寫入原範圍外的內容。

學習者通過的條件是：交接中保留檢查點、部分 diff、缺失證據、權限邊界、復原路徑和
精確的未知說明。繼續堆疊編輯，是這項實驗的失敗。

## 先完成一個小而完整的切片

初學者不必從網站、程式碼或發佈開始。選一個自己能檢查的短文字、一個本機 README，或一組已經允許使用的公開來源。目標不是讓模型「做很多」，而是完成一次從定義到交接都看得見的閉環。

```text
結果：讓一段 120 字以內的說明讓新讀者能找到第一步。
輸入：原文、讀者是誰、一個已知問題。
允許：只讀原文；提出計劃；確認後只編輯該文字。
不允許：連網、登入、安裝、傳送、發佈或改動其他檔案。
檢查：儲存修改前後文字；讓另一人或你自己按「能否找到第一步」檢查一次。
交接：改了什麼、沒有改什麼、檢查結果、仍未知什麼。
```

把這七個階段走一遍：先定義讀者和結果；計劃一處改動；儲存原文作為檢查點；編輯；比對前後；讓新的視角審查；交接給下一位或明天的自己。若需要更多資料或外部動作，先停在 `blocked`，不要為了讓流程看起來完成而擴大權限。

### 兩次嘗試怎樣才可比較

若你要比較「直接讓模型改」與「先寫協議再改」，必須固定原文、目標、允許動作、時間限制和檢查規則。記錄首次輸出、實際耗時、返工次數、diff、檢查結果和未知項。原文、模型、工具、權限或環境改變時，寫 `not_comparable`；一次更快或更順眼的結果不等於普遍效率或模型更好。

## 遷移

把這套生命週期應用到一個非程式碼任務，如研究簡報、行銷頁面或設計交接。對每個階段寫出
...
| Cloud 工作有不同的設定、Agent、審查和後續邊界 | [Codex Cloud](https://learn.chatgpt.com/docs/cloud.md) | 2026-08-09 | 產品說明；帳戶、組織、環境和目前 UI 仍需另行檢查 | `facts-maintainer`／2026-09-09 |
| 容量中斷可能讓依賴任務狀態不明 | [FP-09 / issue #33865](https://github.com/openai/codex/issues/33865) 與[現場研究](../evidence-library-ZHTW.md#source-notes) | 2026-08-09 | 公開使用者報告；沒有本機重現或普遍佇列結論 | `curriculum-maintainer`／2026-09-09 |
...
| 驗證不得靜默擴大為安裝或持久環境改動 | [FP-11 / issue #37677](https://github.com/openai/codex/issues/37677) 與[現場研究](../evidence-library-ZHTW.md#source-notes) | 2026-08-09 | 公開使用者報告；不是官方策略或本機重現 | `curriculum-maintainer`／2026-09-09 |

生命週期原則應當相對穩定；產品工作面、模型名稱、審批預設值、命令參數、認證行為和
外部服務屬於易變事實。任一項變化時，重新整理第一方記錄，然後審查本章、相關 Lab、Skill、
評測夾具和站點路徑。

## 驗收清單

- [ ] 我能寫出帶目標、範圍、非目標、驗收、權限、證據和回滾來源的任務定義。
- [ ] 我能解釋為何面對同一結果，垂直切片比橫向分層更早產生證據。
- [ ] 我能建立一個檢查點，讓另一人無需讀取原對話也能復原。
...
它不是已驗證譯文，也不表示課程已經通過獨立中文審校或學習者驗證。
...
<nav class="chapter-navigation" aria-label="章節導覽">
  <table role="presentation" width="100%">
    <tr>
      <td align="left"><a data-chapter-nav="previous" href="07-skills-plugins-and-tools-ZHTW.md" aria-label="上一章：第 7 章 · Skill、Plugin、MCP 和工具如何分工">← 上一章<br><strong>第 7 章 · Skill、Plugin、MCP 和工具如何分工</strong></a></td>
      <td align="right"><a data-chapter-nav="next" href="09-verification-and-recovery-ZHTW.md" aria-label="下一章：第 9 章 · 驗證、懷疑與恢復">下一章 →<br><strong>第 9 章 · 驗證、懷疑與恢復</strong></a></td>
    </tr>
  </table>
</nav>
