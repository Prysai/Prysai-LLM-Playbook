...
**狀態：** `candidate`。本章定義了攜帶證據的工作流程及復原規則，
但比較實驗尚未執行。案例材料是專案自有的教學材料，不是一次真實 Codex 執行、
客戶委託或正式發布的記錄。

## 本章要解決的問題

讓 Codex「開始寫」通常不難；要把一件有用的工作真正收尾，則是另一回事。

任務表面上看似順利，目標卻可能仍然模糊，範圍正在擴大，檢查跑在錯誤的檔案上，
...
儲存庫還提供一個可丟棄、專案自有的案例，把同一套紀律帶到非程式碼交付物：
...
|---|---|---|---|
| 所選模型變得不可用，任務停止 | 報告者觀察到了容量錯誤和被中斷的任務 | 佇列語意、伺服器端原因，或所有帳戶和版本的行為 | 凍結後續提示；重試前檢查 diff、日誌和最後接受的檢查點 |
...
| 驗證擴大為強制重灌 | Agent 可能把「確認它能用」理解為可以改動持久環境 | 報告描述所有 Agent，或重灌永遠錯誤 | 分開記錄原始碼改動、測試、安裝、重啟、部署和線上驗證；持久改動前先詢問 |
...
這七個階段是組織工作的教學模型，不承諾每個 Codex 工作面都會顯示完全相同的
介面狀態。

| 階段 | 它回答的問題 | 常見允許工作 | 出口證據 | 何時停止 |
|---|---|---|---|---|
| 定義 | 想要什麼結果、給誰、邊界是什麼？ | 閱讀規則、目標、輸入、風險和非目標 | 任務協議與驗收說明 | 缺失輸入會改變範圍、風險、權限或驗收 |
| 計劃 | 最小且有用的工作順序是什麼？ | 對應依賴、選擇切片、暴露高風險未知項 | 有序計劃、切片邊界與檢查 | 計劃只是橫向分層，不能獨立檢查結果 |
...
階段來填補缺口；更長的計劃不能替代缺少的權限、檔案或測試結果。
...
最後一句比前四句嚴格得多。構建通過很有價值，但不會自動成為執行時、視覺、安全或
使用者驗收證據。
...
例如，「讀者能開啟一章並找到實驗」常常比「遷移整本書的導覽」更適合作為第一個
...
檢查點是可復原的狀態說明，而不只是時間戳。它應讓下一次決策無需相信之前的聊天
歷史。
...
「繼續」不是復原計劃。它沒有指出最後接受的狀態，不能防止重複副作用，也不能解釋
為什麼同一動作現在值得再做一次。
...
把沉默當作觀察，不當作成功訊號。啟動長命令前，先定義預期輸出、合理的等待時間和
中斷路徑。等待到期時：
...
交付不是生命週期的終點。若輸出包含易變的模型、工具、權限、命令或服務事實，
...
是不同工作面，有不同證據。文件本身只是日期化的產品來源，不證明某個帳戶或工作區
擁有存取權。
...
選擇一個 Skill 不會讓任務自動完成，也不會給該 Skill 呼叫另一個工具或啟動獨立工作流程
的權限。
...
學習者通過的條件是：交接保留檢查點、部分 diff、缺失證據、權限邊界、復原路徑和
...
把這七個階段走一遍：先定義讀者和結果；計劃一處改動；儲存原文作為檢查點；編輯；比對前後；讓新視角審查；交接給下一位或明天的自己。若需要更多資料或外部動作，先停在 `blocked`，不要為了完成流程而擴大權限。
