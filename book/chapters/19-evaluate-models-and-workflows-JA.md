...
FP-08（モデルと provider の設定が一致しない）、FP-09（容量不足や待ち行列による中断）、FP-10（validation コマンドが `Working` のまま残る）、および [FUP-05（長時間イベントがない後にエラーになり、自動 retry された事例）](../../docs/research/field-problems-follow-up-2026-08-10.md) は、公開された利用者報告から整理したものです。公式な根本原因の認定でも、このプロジェクトでの再現でも、すべてのアカウントに当てはまる結論でもありません。
