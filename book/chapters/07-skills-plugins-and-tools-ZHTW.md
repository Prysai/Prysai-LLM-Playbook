...
「我需要一個 Skill」並不總是正確的診斷。Skill、Plugin、MCP 伺服器、連接器、指令碼、範本和一般文件，解決的是不同問題。把它們當成可以互換的標籤，人們就會安裝超出任務需要的能力，讓上下文更難檢查，也會在不知不覺中擴大可能的外部副作用。

真正有用的問題不是「哪個目錄裡的 Skill 最多」，而是：

> 這項任務缺少什麼？哪一種最小能力可以填補這個缺口，同時讓權限、授權條款、依賴和證據仍然可控？

## 學習目標

完成本章後，你應該能夠：

- 解釋方法、連線、執行和分發四層之間如何分工；
...
專案的 [Codex 現場研究](../evidence-library-ZHTW.md#source-notes) 記錄了兩份公開報告。它們適合作為症狀，而不是官方根因分析或本機復現：

| 公開症狀 | 報告者觀察到什麼 | 它**不能**證明什麼 | 第一個安全響應 |
|---|---|---|---|
| 使用者 Skill 作為一般檔案能工作，但改成檔案符號連結後不再被發現 | 檔案表示方式改變後，發現結果也改變；報告還比較了硬連結 | 每個 Skill 掃描器、作業系統或發行版本都有同一缺陷 | 儲存精確的檔案表示方式和工作面；在隔離測試中比較一般檔案與連結，再記錄結果 |
| 顯式使用 Skill 依賴一個隱式的可用清單 | 使用者無法把顯式請求視為獨立於目前工作面清單的行為 | 該報告描述通用路由規則或官方產品保證 | 分別儲存可見清單、精確請求、工作階段和已載入資源的證據 |

這些報告讓一條實際邊界變得可見：儲存庫裡有一條路徑，不等於目前的宿主已發現這個 Skill；有一個可見名稱，不等於本工作階段已載入它；載入完成也不等於它的外部依賴或權限已經正常運作。
...
對於每次都應以確定方式執行的重複邏輯，指令碼通常更合適；穩定的輸出格式更適合範本；只需在特定情況下閱讀的背景知識，更適合放在文件裡。當方法會反覆出現、但仍需要依上下文判斷時，Skill 才真正有價值。
...
3. 如果任務需要外部資料或外部動作，再問連接器或 MCP 伺服器是否真的必要。
4. 如果轉換是確定性的，優先使用指令碼。
5. 如果多種能力必須一起交付，再把 Plugin 當作分發層考慮。
6. 只有這之後，才決定是否安裝、認證或開放額外權限。

這個順序刻意保守。大型目錄會讓任務看似更有能力，卻讓實際的依賴和權限圖更難看清。
...
- **權限與副作用：** 它能讀取或寫入什麼？是否需要網路或帳戶？能否傳送、發布、刪除、修改，或以其他方式改變外部系統？
- **驗證與維護：** 隔離測試能否涵蓋正例、邊界、失敗和遷移案例？誰來核准、負責、備份、更新，並演練回滾？

外部目錄的條目數量不是品質指標。自動化套件同樣可能帶來帳戶、網路和第三方服務風險；每個候選項都要依據自身證據審查。

### Plugin 包含什麼，以及支援到哪裡為止

官方 [Plugins 文件](https://learn.chatgpt.com/docs/plugins.md) 把 Plugin 描述為一個可安裝的能力套件，其中可以包含 Skills、Connectors，或兩者。一個 Connector 可由 MCP 伺服器支援，為外部系統提供工具、共享資訊或動作。因此 Plugin 是分發和組合層，不等於授權。
...
每個箭頭都是獨立宣告。官方 Plugin 記錄還說明，「Sign in with ChatGPT」並不會自動授予 Plugin 資料存取權或核准動作；請求的權限仍需另外審查和核准。目前的來源記錄將這些邊界關聯到 `OF-015`、`OF-016`、`UF-001`、`UF-003` 和 `LB-002`；在改變受影響章節或實驗前，先檢視[事實影響登記表](../../docs/governance/fact-impact-registry.yaml)。

2026-08-10 查閱的官方 Skills 和 Plugins 材料，還將自動匹配與顯式選擇描述為不同入口：ChatGPT 使用 `@` 提及，Codex 使用 `$` 提及；文件把安裝後的新 chat 或 CLI 工作階段列為流程的一部分。這些是易變的產品事實，不是 Skill 自動附帶的權限。一次本機檢查應記錄工作面、工作階段、精確呼叫字串、載入資源、行為輸出和結果驗證。本儲存庫沒有收集這類執行時記錄，因此相關狀態仍是 `not_observed`。

## 4. 採用前審查包

安裝前，產出一份 `skill-adoption-decision.md` 記錄，而不是寫一句「已檢查授權條款」就結束。最低應記錄：
...
安裝也是可觀察的動作。目標路徑和成功安裝日誌可以支援 `installed-candidate`，卻不能跳過發現、載入、採用或行為驗證。

### 兩個採用決策示例

- **推薦項：** S05 的 `code-review-and-quality` 是合併審查任務中一個合理的 `recommendation-only` 候選。來源為 `https://github.com/addyosmani/agent-skills` 的本地歸檔，SHA-256 證據為 `6EEDBE7D2EA3A82417781D879785BD501FBDE21275627F557DE4B76560BA1250`，且有儲存庫層級的 MIT 訊號。觸發條件是固定 diff 和審查請求；它不應只因生成新功能或審查基線尚未定義就觸發。巢狀依賴、目標 Skill 的完整資產集、實際權限和回滾仍未審查，因此正確下一步是只讀審查或離線隔離試驗，不是核准安裝。所有者為 Prysai LLM Playbook 維護組。
- **阻塞變體：** S06 的 `webapp-testing` 必須保持 `blocked`。它來自 `https://github.com/composio-community/awesome-codex-skills` 的本地歸檔，SHA-256 為 `D3DA83ED9D474690E7FF235351376114972840C78BC319CBCB8F89CBD704608E`。庫存確認根目錄有 Apache-2.0 訊號，但沒有確認每個巢狀 Skill、指令碼與資產都具有一致的授權條款或 NOTICE 覆蓋。如果目標安裝路徑、設定備份與復原檢查也不清楚，光是存在 `SKILL.md` 還不夠。只有完成逐項授權條款審查並具備可演練的回滾方案，才能解除阻塞。在此之前不要下載、安裝，或把它描述為已發現或可用。
...
領域 Skill 負責它的方法；Task Protocol 負責執行邊界；Evidence Review 檢查已有宣告；Workflow Orchestrator 負責階段和檢查點。一個 Skill 不會因為被呼叫就獲得另一個 Skill 的權限，也不應遞迴啟動完整編排。

## 7. 實驗：比較三種能力組合

### 準備

選擇一個本機、低風險、可逆的任務。準備任務協議、兩個固定版本的候選 Skill，以及一個會要求外部連線的模擬選項。其中一個候選項適合繼續隔離審查；另一個應因授權條款、NOTICE 覆蓋或回滾不清而被拒絕。不要上傳真實資料、傳送訊息、寫入第三方服務或認證外部帳戶。為每種組合分配 `run-id`，同時保持任務文字與驗收量表不變。
...
儲存三種方式、各自的 `run-id`、兩份 `skill-adoption-decision.md`、依賴和權限表、授權條款發現、模擬或實際輸出、驗證結果，以及明確的「未執行外部動作」清單。一條完整的記錄必須讓來源與版本可檢查；將授權條款結論指向實際檔案；命名安裝、備份與回滾目標；識別所有者和審批點；涵蓋正例、邊界、失敗和遷移行為；並保留不需要額外連線的基線。模擬呼叫必須標為模擬，不能報告為成功的執行時呼叫。
...
學習者通過的條件是：識別重疊性，拒絕不必要的權限，將不清楚的候選項標記為 `blocked`，並保留只使用任務協議或一個 Skill 的基線。
...
Skill、Plugin、連接器、MCP、manifest、認證和呼叫細節都可能變化。官方頁面或目前工作面發生變化時，先重新整理第一方記錄，再審查事實影響登記表、本章、相關 Labs、Skills、評測 fixture 和站點路徑。將官方產品描述、社群症狀和本機執行時證據寫在不同句子中。
