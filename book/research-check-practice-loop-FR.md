...
publier, acheter, recruter, diagnostiquer ou agir.

## 1. Figer une décision
```text
Je dois prendre cette décision : [décision]. Aide-moi à la transformer en une
question à laquelle on peut répondre. Indique le responsable de la décision, la
limite de date, ce qui changerait la décision et ce qui est hors périmètre. Ne
cherche pas, ne recommande pas, ne classe pas et n’invente pas de faits. Pose une
seule question si un détail décisif manque.
```

## 2. Nommer les affirmations et les propriétaires des sources
```text
Pour la question délimitée, liste au plus trois affirmations importantes et le
type de source qui pourrait être responsable de chacune : page officielle,
recherche primaire, loi ou politique, jeu de données de première partie ou
registre institutionnel nommé. Dis ce que chaque type ne peut pas prouver.
...
conflit ou lacune | date d’accès | prochaine vérification. Marque comme inconnu
ce qui n’apparaît pas. Ne dis pas avoir ouvert un lien si son contenu n’est pas
...
## 5. Chercher une exception qui changerait la décision
```text
Pour le registre actuel, nomme une révision plus récente plausible, une exception
ou un propriétaire de source contradictoire qui pourrait changer la décision.
Explique pourquoi cela compte et quelle est la plus petite vérification permise
