...
2. [通用 LLM 第一次練習（Foundation Core 的遷移參考）](routes/universal-core-foundations-ZHTW.md)
3. [第一次安全改動](routes/first-safe-change-ZHTW.md)

英文五單元 Foundation Core 是規範源；這條繁體中文四單元路線只是遷移期間的補充參考。在繁體中文重譯完成並通過獨立審校之前，繁體中文主線不會把它標示為五單元課程或已驗證內容。
提示卡、語言練習、工作更新和研究核查是主線之後的**可選應用練習**，不替代 LLM 基礎課。

## 先認識幾個常用詞

- **工作面**：實際使用模型與工具的入口或介面，例如終端機、瀏覽器或 Codex 視窗；它不等於已經取得權限。
...
- [繁體中文通用 LLM 第一次練習：安全文字任務（Foundation Core 的遷移參考）](routes/universal-core-foundations-ZHTW.md)
...
如果你還沒有專案，也不準備讓 AI 操作檔案，可以把[通用 LLM 第一次練習](routes/universal-core-foundations-ZHTW.md)作為英文五單元 Foundation Core 的遷移參考：它只使用一則虛構通知，練習把目標、材料、檢查和停止點寫清楚。它是候選練習，不代表不同平台行為相同，也不證明學習效果。
