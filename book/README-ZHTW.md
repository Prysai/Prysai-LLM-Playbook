...
2. [通用 LLM 第一次練習](routes/universal-core-foundations-ZHTW.md)
3. [第一次安全改動](routes/first-safe-change-ZHTW.md)

英文 Foundation Core 是規範來源。在繁體中文重譯完成並通過獨立審校前，不把它當作繁體中文主線中已完成的內容。
...
- [繁體中文通用 LLM 第一次練習：安全文字任務](routes/universal-core-foundations-ZHTW.md)
...
如果你還沒有專案，也不準備讓 AI 操作檔案，先做[通用 LLM 第一次練習](routes/universal-core-foundations-ZHTW.md)：它只使用一則虛構通知，練習把目標、材料、檢查和停止點寫清楚。它是候選練習，不代表不同平台行為相同，也不證明學習效果。
