...
它是證據導航，不是原始記錄的替代品。來源記錄只能支援其中明確寫出的範圍；它不能證明學習效果、當前產品行為、模型質量或普遍安全性。
...
完整的維護術語記錄標識為 `CONTEXT.md`。產品名稱、選單、預設值、價格、額度和許可權都是易變事實，使用前需要帶日期的一手來源。
...
| 標籤 | 可以支援什麼 | 不能支援什麼 |
| --- | --- | --- |
| 官方基線 | 在宣告範圍內、帶日期的產品事實 | 所有賬號或未來版本的承諾 |
