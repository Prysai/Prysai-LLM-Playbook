...
2. [第一次通用 LLM 练习](book/routes/universal-core-foundations-ZH.md)
...
这个项目的价值不在于再增加一串 AI 术语，而在于让你留下可以检查、也能带到下一个任务里的东西。五单元基础路线围绕三项由学习者自己完成的产出设计：

- **一张有边界的任务卡：** 写清目标、提供的上下文、允许的帮助、限制、检查方式和停止条件；
- **一份结果检查记录：** 记录模型提出或改动了什么、你检查了什么、保留了哪些证据，以及哪些内容仍然未知；
- **一次迁移尝试：** 把同一套方法用到新任务上，并明确决定继续、修正还是停止。

这些是路线的设计目标，不是已经测得的学习结果。项目仍为 `candidate`；学习完成、迁移和长期保持的证据仍是 `not_run`。
...
当前项目处于 v0.1 的产品地基阶段：22 章书籍结构、18 个实验、真实问题研究、26 个候选 Skill（25 个原创方法和 1 个已审查的外部编辑 Skill）以及约 40 项评测夹具已经建立。目录、结构与静态契约检查不等于学习者已经完成、迁移或掌握这些内容。

书籍规范源为英语（`EN`），公开展示页默认使用英文，并提供八种语言切换。简体中文已经具备 22 章和 18 个实验的 `-ZH` 候选文件及同语言阅读路径；这表示课程单元可沿中文路线往返阅读，并不等于完整中文课程已经通过独立语言审校或学习者验证。Skill、评测、研究档案和部分补充读物仍在迁移中。多语言架构要求每个 reader-facing 文件带明确后缀，同一内容 ID 的链接保持当前语言；在独立语言审校完成前，现有译文保持 `in-progress`，不能被宣传为完整八语言发行。
...
| 繁體中文 | 40 / 40 | 22 章和 18 个实验均有候选译文；独立语言审校待完成。 |
| Français | 40 / 40 | 22 章和 18 个实验均有法语候选译文；独立法语审校待完成。 |

八种语言线均已具备 40 个课程单元的文件与路径。这里说的是结构覆盖，**不**表示八种语言都已完成独立审校、文化适配、学习效果验证或正式发行。页面中的 `available / 40` 是路径数量，不是准确度、自然程度、学习效果或发行状态的分数。
