...
- `visuals.html?lang=<locale>` 是独立的视觉导览入口：六阶段工作闭环图、十一步视觉导览路线、五步首次尝试回执图、五阶段证据成熟度图、二十二张项目原创教学图和每张图旁的本地化说明都保留同一语言路径；JavaScript 失败时，各路线的有序文字回退仍可直接阅读；
...
- `foundation-lens` 先展示一张最小安全 LLM 闭环图和四张检查图；`visual-cases` 另链接四张项目原创 SVG 教学卡。它们解释方法，不证明 Skill runtime、客户需求、市场效果或生产准备度。
- Reader 页面在页眉提供通往视觉导览的当前语言入口；开头段落后先提供当前页面对应的项目原创教学图，再提供一个渐进增强的概念图：静态有序标题列表始终可用，脚本运行时会在滚动阅读中高亮当前小节，点击节点会回到对应标题；侧栏保留同一结构的图形化版本。图解的本地化标题、替代文本、图注和文字等价说明与页面语言同步；概念图只由当前语言的页面标题生成，不替代正文，也不宣称理解或完成证据。
- 教学 SVG 采用“局部缩略图 + 本地化说明 + 完整尺寸链接 + 文字替代”的组合。手机端不依赖拖拽、悬停或外部图表库；`node scripts/browser_smoke.mjs` 会检查八种语言、390px 视口、图示加载、概念图入口和横向溢出。
...
6. 检查 18 个实验卡片、索引链接和窄屏布局与菜单展开；菜单打开后按 Escape，确认焦点回到菜单按钮；打开 `visuals.html` 的八种语言，确认六个工作闭环节点、十一步视觉路线、五步首次尝试回执、五阶段证据成熟度图、二十二张图板、文字回退、图片加载和当前语言 Reader 链接；Reader 还要检查跳到正文、加载状态、错误状态和中文外壳文案。快速回归可运行 `npm run test:visuals`，它覆盖八种语言、动态图板数量、证据成熟度最后节点、390/360px 横向溢出和关闭 JavaScript 后的文字回退；完整 `npm run test:browser` 仍是更广的页面与 Reader 烟测；
