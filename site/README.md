...
- `visuals.html?lang=<locale>` 是独立的视觉导览入口：六阶段动态路线图、十二张项目原创教学图和每张图旁的本地化说明都保留同一语言路径；JavaScript 失败时，六阶段有序列表仍可直接阅读；
...
6. 检查 18 个实验卡片、索引链接和窄屏布局与菜单展开；菜单打开后按 Escape，确认焦点回到菜单按钮；打开 `visuals.html` 的八种语言，确认六个地图节点、十二张图板、文字回退、图片加载和当前语言 Reader 链接；Reader 还要检查跳到正文、加载状态、错误状态和中文外壳文案；
