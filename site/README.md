...
- `visuals.html?lang=<locale>` 是独立的视觉导览入口：六阶段工作闭环图、十一步视觉导览路线、五步首次尝试回执图、十八张项目原创教学图和每张图旁的本地化说明都保留同一语言路径；JavaScript 失败时，各路线的有序文字回退仍可直接阅读；
...
6. 检查 18 个实验卡片、索引链接和窄屏布局与菜单展开；菜单打开后按 Escape，确认焦点回到菜单按钮；打开 `visuals.html` 的八种语言，确认六个工作闭环节点、十一步视觉路线、五步首次尝试回执、十八张图板、文字回退、图片加载和当前语言 Reader 链接；Reader 还要检查跳到正文、加载状态、错误状态和中文外壳文案；
