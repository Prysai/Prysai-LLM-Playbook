...
- Reader 页面在开头段落后先提供当前页面对应的项目原创教学图，再提供一个渐进增强的概念图：静态有序标题列表始终可用，脚本运行时会在滚动阅读中高亮当前小节，点击节点会回到对应标题；侧栏保留同一结构的图形化版本。图解的本地化标题、替代文本、图注和文字等价说明与页面语言同步；概念图只由当前语言的页面标题生成，不替代正文，也不宣称理解或完成证据。
