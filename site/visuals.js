(() => {
  ...
  const ROUTE_COPY = {
    en: { routeEyebrow: 'A guided visual route', routeTitle: 'Know what each picture is for.', routeIntro: 'Follow the route from your purpose to a checked decision. Select a step to see what it explains, what to ask next, and where to continue.', routeAria: 'Eleven steps in the visual route', routeProgress: 'Reading progress', routeSelected: 'Current visual step', routeNext: 'Next question', routeOpen: 'Jump to this section', routeFallback: 'Read the visual route as text', routeFallbackIntro: 'The same order remains available without JavaScript or pointer interaction.' },
    zh: { routeEyebrow: '视觉导览路线', routeTitle: '先知道每张图要解决什么问题。', routeIntro: '从你的目的走到一个可检查的决定。选择一个步骤，查看它解释什么、下一步该问什么，以及应该继续到哪里。', routeAria: '视觉导览路线的十一个步骤', routeProgress: '阅读进度', routeSelected: '当前视觉步骤', routeNext: '下一个问题', routeOpen: '跳到这部分', routeFallback: '按文字阅读视觉路线', routeFallbackIntro: '即使没有 JavaScript 或鼠标操作，也可以按同样顺序阅读。' },
    es: { routeEyebrow: 'Recorrido visual guiado', routeTitle: 'Entiende para qué sirve cada imagen.', routeIntro: 'Sigue el recorrido desde tu propósito hasta una decisión comprobable. Elige un paso para ver qué explica, qué preguntar después y dónde continuar.', routeAria: 'Once pasos del recorrido visual', routeProgress: 'Progreso de lectura', routeSelected: 'Paso visual actual', routeNext: 'Siguiente pregunta', routeOpen: 'Ir a esta sección', routeFallback: 'Leer el recorrido visual como texto', routeFallbackIntro: 'El mismo orden está disponible sin JavaScript ni interacción con el puntero.' },
    ja: { routeEyebrow: '視覚ガイドの道筋', routeTitle: '各図が何のためにあるかを先に知る。', routeIntro: '目的から、確認できる判断までの道筋をたどります。段階を選ぶと、何を説明し、次に何を問い、どこへ進むかが分かります。', routeAria: '視覚ガイドの11段階', routeProgress: '読書の進み具合', routeSelected: '現在の視覚ステップ', routeNext: '次の問い', routeOpen: 'このセクションへ移動', routeFallback: '視覚ルートをテキストで読む', routeFallbackIntro: 'JavaScript やポインター操作がなくても、同じ順序で読めます。' },
    ko: { routeEyebrow: '안내형 시각 경로', routeTitle: '각 그림이 무엇을 위한 것인지 먼저 확인하세요.', routeIntro: '목적에서 확인 가능한 결정까지 경로를 따라가세요. 단계를 선택하면 무엇을 설명하는지, 다음에 무엇을 물을지, 어디로 이어지는지 볼 수 있습니다.', routeAria: '시각 경로의 열한 단계', routeProgress: '읽기 진행률', routeSelected: '현재 시각 단계', routeNext: '다음 질문', routeOpen: '이 섹션으로 이동', routeFallback: '시각 경로를 텍스트로 읽기', routeFallbackIntro: 'JavaScript나 포인터 조작 없이도 같은 순서로 읽을 수 있습니다.' },
    de: { routeEyebrow: 'Geführte visuelle Route', routeTitle: 'Verstehe zuerst, wofür jedes Bild da ist.', routeIntro: 'Folge dem Weg von deinem Zweck zu einer prüfbaren Entscheidung. Wähle eine Stufe, um Erklärung, nächste Frage und Anschluss zu sehen.', routeAria: 'Elf Schritte der visuellen Route', routeProgress: 'Lesefortschritt', routeSelected: 'Aktueller visueller Schritt', routeNext: 'Nächste Frage', routeOpen: 'Zu diesem Abschnitt springen', routeFallback: 'Die visuelle Route als Text lesen', routeFallbackIntro: 'Dieselbe Reihenfolge bleibt ohne JavaScript und Zeigerbedienung verfügbar.' },
    'zh-tw': { routeEyebrow: '視覺導覽路線', routeTitle: '先了解每張圖要解決什麼問題。', routeIntro: '從你的目的走到一個可檢查的判斷。選擇一個步驟，查看它說明什麼、下一步要問什麼，以及應該繼續到哪裡。', routeAria: '視覺導覽路線的十一個步驟', routeProgress: '閱讀進度', routeSelected: '目前視覺步驟', routeNext: '下一個問題', routeOpen: '跳到這個區段', routeFallback: '依文字閱讀視覺路線', routeFallbackIntro: '即使沒有 JavaScript 或指標操作，也能依相同順序閱讀。' },
    fr: { routeEyebrow: 'Parcours visuel guidé', routeTitle: 'Sachez à quoi sert chaque image.', routeIntro: 'Suivez le parcours, de votre objectif jusqu’à une décision vérifiable. Sélectionnez une étape pour voir ce qu’elle explique, la question suivante et la suite du parcours.', routeAria: 'Onze étapes du parcours visuel', routeProgress: 'Progression de lecture', routeSelected: 'Étape visuelle actuelle', routeNext: 'Question suivante', routeOpen: 'Aller à cette section', routeFallback: 'Lire le parcours visuel en texte', routeFallbackIntro: 'Le même ordre reste disponible sans JavaScript ni interaction avec le pointeur.' },
  };
  ...
  function renderVisualRoute() {
    ...
    const detailProgress = query('[data-visual-route-progress]');
    ...
    if (detailProgress) detailProgress.textContent = `${String(VISUAL_ROUTE_STEPS.indexOf(step) + 1).padStart(2, '0')} / ${VISUAL_ROUTE_STEPS.length}`;
