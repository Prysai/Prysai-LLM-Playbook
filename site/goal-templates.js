/* Generated by scripts/build_goal_templates.py. Do not edit by hand. */
 ...
 "goals": {
  ...
  "zh-tw": {
    ...
    "fields": [
     {
      "key": "language",
      "label": "要練習的語言",
      "placeholder": "例如：西班牙文、法文、德文"
     },
     {
      "key": "scene",
      "label": "一個情境",
      "placeholder": "例如：確認讀書小組時間、規劃作業、課堂討論"
     }
    ],
    "template": "用 {language} 進行一次四輪打字對話，情境是{scene}。你扮演對方並先開口，一次只問一個簡短問題，我每輪回覆一句。\n\n只使用虛構資訊。不要索取或接受真實姓名、學校、行事曆、帳號、地址、聯絡方式或付款資訊。開始前先顯示這份固定檢查：學習者回覆四輪；所需資訊已傳達；一處歧義已解決；表達足以讓對話繼續。在我回覆前不要翻譯、教學或展示參考答案。保留我的首次嘗試。只修正第一個阻礙理解的錯誤：說明錯誤類型，給一個部分提示，只有在我仍然無法繼續時才給一個完整片段。保留兩次嘗試，不要把一次對話稱為流利、口語對話或聽力／發音能力證據。",
    "path": "../book/communication-clinic-ZHTW.md#language-practice-route"
   },
   "work": {
    "fields": [
     {
    ...
    "template": "我需要把下面的要點整理成一則給{audience}的簡短更新。\n\n原始要點：\n{facts}\n\n要求：\n- 先列出你能從原始要點確認的事實；\n- 不要補充日期、承諾、完成狀態或原因；\n- 給一個不超過 120 字的版本和一個更正式的版本；\n- 最後列出我傳送前必須親自確認的兩件事。",
    ...
    "fields": [
     {
      "key": "question",
      "label": "你想核查的主張",
      "placeholder": "例如：這個產品在 2026 年調整了定價"
     },
     {
      "key": "material",
      "label": "你可以分享的材料",
      "placeholder": "網址、標題、摘錄，或「沒有」"
     }
    ],
    "template": "我想在重複一句主張之前先核查它。\n\n問題：{question}\n我提供的材料：{material}\n\n不要編造來源，也不要聲稱你開啟過無法存取的內容。製作一張三列表格：可能的主張、提供的來源或「缺少」、還需要核查什麼。區分事實、報告與推論。如果材料缺少、互相矛盾、涉及個人或高風險，請停止並告訴我最小且安全的下一步。",
   ...
   "interview": {
    ...
    "template": "我想演練一個面試回答。\n\n問題：{question}\n回答時間：{time}\n\n先讓我用自己的話在時限內回答。開始前先說明可見檢查：一個具體例子、一個決定、一個結果、結構清楚。我的回答之後，最多指出一個關鍵缺口並給一個部分提示。然後讓我修改，最後用同樣的檢查、不給提示地問一個變式問題。不要寫參考答案、預測其他面試問題或承諾結果。",
    "path": "../skills/prysai-interview-rehearsal/SKILL.md"
   },
   "task": {
    "fields": [
     {
      "key": "goal",
      "label": "你想完成什麼",
      "placeholder": "例如：修復說明頁上的一個失效連結"
     },
     {
      "key": "context",
      "label": "相關上下文",
      "placeholder": "例如：檔案是 docs/help.md；我可以在本機修改"
     }
    ],
    "template": "我需要把一個任務變成清楚的請求。\n\n目標：{goal}\n上下文：{context}\n\n請寫一份任務協議，欄位恰好包括：目標、上下文、允許行動、驗收檢查、證據、停止條件。不要行動、不要瀏覽、不要使用工具。只有當缺少欄位會改變風險或驗收檢查時才問一個問題。保持範圍小且可回復。",
    "path": "../book/chapters/03-task-protocol-ZHTW.md#core-task-contract"
   },
   "codex": {
    "fields": [],
