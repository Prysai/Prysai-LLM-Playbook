...
Object.assign(copy.en, {
  ...
  spanishCardTitle: 'Complete one short typed Spanish hotel check-in.',
  spanishCardIntro: 'This text-only card uses fictional travel details, waits for your typed attempt, and limits help to one meaning-blocking error.',
  spanishCardStepOne: 'Copy the card exactly as written. It already sets a fictional typed hotel check-in.',
  spanishCardStepTwo: 'Paste it into any text chat. Do not add a real name, booking, passport, or payment detail.',
  ...
  spanishPromptText: 'Run one four-minute typed Spanish hotel check-in with exactly four learner turns. You are the receptionist and write first. Use only short present-tense questions. I will type one answer after each question.\n\nFictional guest card: Ana Torres; two nights; single room; breakfast included; ask whether breakfast starts at 7:00 or 7:30. I may use the card and look up at most three single words. Do not request or accept a real name, booking number, passport, address, contact, or payment detail.\n\nBefore turn one, show this fixed rubric: four learner turns; name and two-night stay communicated; single room and breakfast communicated; 7:00/7:30 ambiguity resolved; Spanish understandable enough to continue. Do not teach, translate, or show a model answer before I reply. Preserve my first attempt and record lookups. Correct only the first meaning-blocking error: name the error type, then give a partial cue, then one worked fragment only if I still cannot continue. Ask me to correct it. Keep both attempts and do not call one successful exchange fluency, spoken conversation, or listening/pronunciation evidence.',
...
Object.assign(copy.zh, {
  ...
  spanishCardTitle: '完成一次简短的文字版西班牙语酒店入住对话。',
  spanishCardIntro: '这张纯文字卡可以原样复制：它使用虚构旅行信息，先等待你的文字尝试，并且只处理一个阻碍含义的错误。',
  spanishCardStepOne: '原样复制整张卡。它已经设定了虚构的文字版酒店入住情境。',
  spanishCardStepTwo: '粘贴到任何纯文本对话中。不要加入真实姓名、预订号、护照或支付信息。',
  ...
  spanishPromptText: '进行一次四分钟文字版西班牙语酒店入住对话，学习者恰好打字回答四轮。你是前台接待员并先用文字提问。只用简短的现在时问题。我会在每个问题后打字回答一次。\n\n虚构住客卡：Ana Torres；住两晚；单人房；包含早餐；询问早餐在 7:00 还是 7:30 开始。我可以使用这张卡，并最多查询三个单词。不要索要或接受真实姓名、预订号、护照、地址、联系方式或支付信息。\n\n第一轮前先展示固定量规：四轮学习者回答；姓名和两晚住宿表达清楚；单人房和早餐表达清楚；解决 7:00/7:30 的歧义；西班牙语足以继续文字对话。不要在我回答前教学、翻译或展示模型答案。保留我的初次尝试并记录查询。只纠正第一个阻碍含义的错误：指出错误类型，然后给出部分提示；只有我仍无法继续时才给出一个示范片段。请我自行改正。保留两次尝试，不要把一次成功对话称为流利、真实口语或听力/发音证据。',
...
Object.assign(copy.en, {
  ...
  heroScope: 'Six platforms, one method: the transferable core plus evidence-gated adapters for ChatGPT, Claude / Claude Code, Gemini, DeepSeek, Grok, and the Codex flagship track.',
...
Object.assign(copy.zh, {
  ...
  heroScope: '六个平台，同一套方法：可迁移核心，加上 ChatGPT、Claude / Claude Code、Gemini、DeepSeek、Grok 与 Codex 旗舰轨道的循证适配器。',
...
copy.es = {
  ...
  'spanishCardTitle': 'Completa un registro de hotel en español, breve y por escrito.',
  'spanishCardIntro': 'Esta tarjeta de solo texto usa detalles de viaje ficticios, espera tu intento escrito y limita la ayuda a un error que bloquee el significado.',
  'spanishCardStepOne': 'Copia la tarjeta exactamente como está escrita. Ya plantea un registro de hotel ficticio por escrito.',
  'spanishCardStepTwo': 'Pégala en cualquier chat de texto. No agregues un nombre real, reserva, pasaporte ni dato de pago.',
  ...
  'spanishPromptText': 'Haz un registro de hotel en español por escrito de cuatro minutos con exactamente cuatro turnos del aprendiz. Tú eres el recepcionista y escribes primero. Usa solo preguntas cortas en presente. Yo escribiré una respuesta después de cada pregunta.\n\nTarjeta de huésped ficticia: Ana Torres; dos noches; habitación individual; desayuno incluido; pregunta si el desayuno empieza a las 7:00 o a las 7:30. Puedo usar la tarjeta y consultar como máximo tres palabras sueltas. No solicites ni aceptes un nombre real, número de reserva, pasaporte, dirección, contacto ni dato de pago.\n\nAntes del primer turno, muestra esta rúbrica fija: cuatro turnos del aprendiz; se comunica el nombre y la estadía de dos noches; se comunica la habitación individual y el desayuno; se resuelve la ambigüedad de 7:00/7:30; español lo bastante comprensible para continuar. No enseñes, traduzcas ni muestres una respuesta modelo antes de que responda. Conserva mi primer intento y registra las consultas. Corrige solo el primer error que bloquee el significado: nombra el tipo de error, luego da una pista parcial y, solo si todavía no puedo continuar, un fragmento resuelto. Pídeme que lo corrija. Conserva ambos intentos y no llames a un intercambio exitoso fluidez, conversación oral ni evidencia de comprensión auditiva o pronunciación.',
  ...
  'heroScope': 'Seis plataformas, un mismo método: el núcleo transferible más adaptadores con evidencia para ChatGPT, Claude / Claude Code, Gemini, DeepSeek, Grok y la vía insignia de Codex.',
...
copy.ja = {
  ...
  'spanishCardTitle': '短い文字入力のスペイン語ホテルチェックインを1回完了する。',
  'spanishCardIntro': 'このテキストのみのカードは、架空の旅行情報を使い、あなたの文字入力の試行を待ち、意味を妨げるエラー1つのみにヘルプを限定します。',
  'spanishCardStepOne': 'カードを書かれた通りにコピーしてください。すでに架空の文字入力ホテルチェックインが設定されています。',
  'spanishCardStepTwo': '任意のテキストチャットに貼り付けます。実際の名前、予約、パスポート、支払い情報を追加しないでください。',
  ...
  'spanishPromptText': '学習者のターンがちょうど4回になる、4分の文字入力スペイン語ホテルチェックインを1回実行してください。あなたは受付係で、最初に文字で質問します。短い現在形の質問だけを使ってください。私は各質問の後に1回ずつ文字で答えます。\n\n架空のゲストカード:Ana Torres。2泊。シングルルーム。朝食付き。朝食が7:00か7:30に始まるかを尋ねること。私はカードを使ってもよく、最大3つの単語だけを調べてもかまいません。実際の名前、予約番号、パスポート、住所、連絡先、支払い情報を要求したり受け取ったりしないでください。\n\n最初のターンの前に、この固定ルーブリックを表示してください:学習者のターンが4回。名前と2泊の滞在が伝わる。シングルルームと朝食が伝わる。7:00/7:30の曖昧さが解消される。スペイン語が続けるのに十分理解できる。私が返信する前に、教えたり、翻訳したり、モデルの回答を表示したりしないでください。私の最初の試行を保持し、調べた単語を記録してください。最初の意味を妨げるエラーのみを訂正してください:エラーの種類を指摘し、部分的なヒントを与え、それでも続けられない場合にのみ、完成した断片を1つ示してください。私に訂正を求めてください。両方の試行を保持し、1回の成功したやり取りを、流暢さ、会話、リスニング/発音のエビデンスと呼ばないでください。',
  ...
  'heroScope': '6つのプラットフォーム、ひとつの方法：転移可能なコアに加え、ChatGPT、Claude / Claude Code、Gemini、DeepSeek、Grok、そして Codex 旗艦トラックのエビデンス付きアダプター。',
...
copy.ko = {
  ...
  'spanishCardTitle': '짧은 스페인어 호텔 체크인 하나를 타이핑으로 완료하세요.',
  'spanishCardIntro': '이 텍스트 전용 카드는 가상의 여행 정보를 사용하고, 당신의 타이핑 시도를 기다리며, 도움을 의미를 막는 오류 하나로 제한합니다.',
  'spanishCardStepOne': '카드를 적힌 그대로 복사하세요. 이미 가상의 타이핑 호텔 체크인이 설정되어 있습니다.',
  'spanishCardStepTwo': '아무 텍스트 채팅에 붙여넣으세요. 실제 이름, 예약, 여권, 결제 정보를 추가하지 마세요.',
  ...
  'spanishPromptText': '네 번의 학습자 턴으로 정확히 구성된 4분짜리 스페인어 호텔 체크인 타이핑 연습을 진행하세요. 당신은 프런트 직원이며 먼저 타이핑으로 질문합니다. 짧은 현재형 질문만 사용하세요. 각 질문 후에 제가 답변 하나를 타이핑합니다.\n\n가상 손님 카드: Ana Torres, 2박, 싱글룸, 조식 포함. 조식이 7:00에 시작하는지 7:30에 시작하는지 물어보세요. 저는 카드를 사용할 수 있고 단어를 최대 세 개까지만 찾아볼 수 있습니다. 실제 이름, 예약 번호, 여권, 주소, 연락처, 결제 정보를 요청하거나 받아들이지 마세요.\n\n첫 턴 전에 이 고정 루브릭을 보여주세요. 학습자 턴 4번, 이름과 2박이 전달될 것, 싱글룸과 조식이 전달될 것, 7:00/7:30 모호성이 해소될 것, 계속할 수 있을 만큼 스페인어가 이해 가능할 것. 제가 답하기 전에 가르치거나, 번역하거나, 모델 답변을 보여주지 마세요. 제 첫 시도를 보존하고 찾아본 단어를 기록하세요. 의미를 막는 첫 번째 오류만 교정하세요. 오류 유형을 말하고, 부분 단서를 주고, 그래도 계속할 수 없을 때만 예시 조각 하나를 보여주세요. 제가 고치도록 요청하세요. 두 시도를 모두 보존하고, 성공한 교류 한 번을 유창함, 구어 대화, 듣기/발음 증거라고 부르지 마세요.',
  ...
  'heroScope': '여섯 플랫폼, 하나의 방법: 전이 가능한 핵심과 ChatGPT, Claude / Claude Code, Gemini, DeepSeek, Grok, Codex 플래그십 트랙의 증거 기반 어댑터.',
...
copy.de = {
  ...
  'spanishCardTitle': 'Führe einen kurzen getippten spanischen Hotel-Check-in durch.',
  'spanishCardIntro': 'Diese textbasierte Karte nutzt fiktive Reisedaten, wartet auf deinen getippten Versuch und begrenzt die Hilfe auf einen bedeutungsblockierenden Fehler.',
  'spanishCardStepOne': 'Kopiere die Karte genau so, wie sie geschrieben ist. Sie legt bereits einen fiktiven getippten Hotel-Check-in fest.',
  'spanishCardStepTwo': 'Füge sie in einen beliebigen Text-Chat ein. Füge keinen echten Namen, keine Buchung, keinen Reisepass und keine Zahlungsdaten hinzu.',
  ...
  'spanishPromptText': 'Führe einen vierminütigen getippten spanischen Hotel-Check-in mit genau vier Lernenden-Runden durch. Du bist der Empfang und schreibst zuerst. Verwende nur kurze Fragen im Präsens. Ich tippe nach jeder Frage eine Antwort.\n\nFiktive Gästekarte: Ana Torres; zwei Nächte; Einzelzimmer; Frühstück inklusive; frage, ob das Frühstück um 7:00 oder 7:30 beginnt. Ich darf die Karte nutzen und höchstens drei einzelne Wörter nachschlagen. Fordere keine echten Daten an und akzeptiere sie nicht: Name, Buchungsnummer, Reisepass, Adresse, Kontakt oder Zahlungsdaten.\n\nZeige vor Runde eins dieses feste Raster: vier Lernenden-Runden; Name und Zwei-Nächte-Aufenthalt übermittelt; Einzelzimmer und Frühstück übermittelt; 7:00/7:30-Mehrdeutigkeit geklärt; Spanisch verständlich genug, um fortzufahren. Lehre nicht, übersetze nicht und zeige keine Modellantwort, bevor ich antworte. Behalte meinen ersten Versuch und dokumentiere Nachschlagevorgänge. Korrigiere nur den ersten bedeutungsblockierenden Fehler: Benenne den Fehlertyp, gib dann einen partiellen Hinweis und nur dann ein ausgearbeitetes Fragment, wenn ich immer noch nicht weitermachen kann. Bitte mich, ihn zu korrigieren. Behalte beide Versuche und bezeichne einen erfolgreichen Austausch nicht als Sprachbeherrschung, gesprochene Konversation oder Hör-/Aussprachebeleg.',
  ...
  'heroScope': 'Sechs Plattformen, eine Methode: der übertragbare Kern plus evidenzgestützte Adapter für ChatGPT, Claude / Claude Code, Gemini, DeepSeek, Grok und den Codex-Flaggschiff-Track.',
