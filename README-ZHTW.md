...
2. [通用 LLM 第一次練習（Foundation Core 的遷移參考）](book/routes/universal-core-foundations-ZHTW.md)
...
英文五單元基礎核心課是當前的規範原始檔；這條中文四單元路線只是遷移期間的補充參考，
不能視為五單元課程的中文翻譯。在此之前，這個入口會保持整條路線只連結中文檔案。

西班牙語、工作更新和研究核查卡是學完主線後的**可選應用練習**，不是理解大語言模型的第一課，也不保證效率、流利度或能力提升。

## 這條路線希望你留下什麼

這個專案的價值不在於再增加一串 AI 術語，而在於讓你留下可以檢查、也能帶到下一個任務的成果。五單元基礎路線圍繞三項由學習者自行完成的產出設計：

- **一張有邊界的任務卡：** 寫清目標、提供的脈絡、允許的協助、限制、檢查方式和停止條件；
- **一份結果檢查記錄：** 記下模型提出或改動了什麼、你檢查了什麼、保留了哪些證據，以及哪些內容仍然未知；
- **一次遷移嘗試：** 把同一套方法用在新任務上，並明確決定繼續、修正還是停止。

這些是路線的設計目標，不是已經測得的學習結果。專案仍是 `candidate`；學習完成、遷移和長期保留的證據仍是 `not_run`。
...
- **理解主軸：** 認識 GPT 和模型如何工作，理解上下文、工具、Skill、Agent、權限和驗證如何改變結果。
- **能力主軸：** 從小實驗開始，逐步練習任務表達、工作流設計、Skill 選擇、結果審查和團隊治理。

## 專案由哪些部分組成

下表列出主要目錄的職責。目錄本身是結構性入口，不代表其中的每一個讀者可見的檔案都已經完成八語言遷移。
...
| 評測 | `docs/quality/`、`evals/` | 品質標準、任務夾具和審查記錄 | 判斷內容、Skill 和學習結果是否真的有效 |
| 治理 | `docs/governance/` | 權限、來源、狀態、更新和貢獻規則 | 管理變化與責任邊界 |
...
書籍規範源為英文（`EN`），公開展示頁預設使用英文，並提供語言切換。繁體中文已經具備 22 章和 18 個實驗的 `-ZHTW` 候選檔案及同語言閱讀路徑；這表示課程單元可沿繁體中文路線往返閱讀，並不等於完整繁體中文課程已經透過獨立語言審校或學習者驗證。Skill、評測、研究檔案和部分補充讀物仍在遷移中。多語言架構要求每個讀者可見的檔案帶明確字尾，同一內容 ID 的連結保持當前語言；在獨立語言審校完成前，現有翻譯保持 `in-progress`，不能被宣傳為完整八語言支援。
...
章節 19–22、全部候選 Skill 和模型/工作流評測仍需要 fresh-context 前測與執行日誌。外部材料不會未經來源、授權條款和內容審查直接進入主線；真實問題研究記錄的是使用者報告或社群建議，不自動等同於官方根因，也不等同於本地復現。
...
- [繁體中文書籍目錄](book/table-of-contents-ZHTW.md)
- [通用 LLM 第一次練習（Foundation Core 的遷移參考）](book/routes/universal-core-foundations-ZHTW.md)
- [繁體中文新手提示卡](book/communication-clinic-ZHTW.md)

術語表、治理規則、來源登錄表和評測定義目前仍以 locale-neutral 記錄為主；部分研究檔案和 Skill 說明已有繁體中文候選檔案，但尚未完成獨立語言審校，因此本入口暫不把它們作為已完成的繁體中文正文開放。
