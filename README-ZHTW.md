...
> 簡體中文專案入口（`ZH`）。預設公開語言目標是 English（`EN`）；本檔案是當前中文入口遷移的一部分。
...
當前專案處於 v0.1 的產品地基階段：22 章書籍結構、18 個實驗、真實問題研究、25 個候選 Skill 和約 40 項評測夾具已經建立。目錄、結構與靜態契約檢查不等於學習者已經完成、遷移或掌握這些內容。
