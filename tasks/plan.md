...
本轮完成项目命名收敛、仓库元数据同步、评测入口接入和公开发布门禁修正。当前仓库仍是私有开发仓库；书稿、Skill 和评测中的未验证部分必须继续保持 `draft` 或 `candidate`。

## 阶段任务

### 阶段 1：地基与命名

- [x] 确定对外项目名为 `Codex Field Guide`。
- [x] 使用中文副标题说明学习、实验和实战范围。
