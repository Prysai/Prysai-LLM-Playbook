...
ライセンス：コース本文と教材は CC BY 4.0、スクリプトとツールは Apache-2.0 です（ファイル内に別段の定めがない限り）。[`LICENSE`](LICENSE)、[`LICENSE-CODE`](LICENSE-CODE)、[ライセンス境界（locale-neutral）](docs/sources/licensing.md)を参照してください。
...
このプロジェクトの価値は、AI 用語の一覧を増やすことではなく、あと
から確認でき、次の課題にも持っていける成果物を残すことにあります。
5 ユニットの基礎ルートは、学習者が次の 3 つを自分で作ることを目指します。

- **範囲を区切ったタスクカード：** 目的、提供したコンテキスト、許可する
  支援、制約、確認方法、停止条件を明記する。
- **確認済みの結果記録：** モデルが提案または変更した内容、自分で確認した
  こと、残した根拠、まだ分からないことを記録する。
- **転移の試行：** 同じ方法を新しい課題で試し、続行、修正、停止のどれに
  するかを明確に判断する。

これは設計上の目標であり、測定済みの学習成果ではありません。プロジェクトは
引き続き `candidate` で、学習完了、転移、長期保持の根拠は `not_run` です。
